<?php
class ModelNurseNurse5c extends Model {
	public function addNurse5c($data) {		
		
		if (isset($data['nurse5cform'])) {		
			//$this->db->query("DELETE FROM " . DB_PREFIX . "customer_nurse5c WHERE customer_id = '" . (int)$this->request->get['customer_id'] . "'");
			
			/*
			$totaldata = $this->getNurse5cs((int)$this->request->get['customer_id']);
			
			$sid = array();
			foreach ($data['nurse5cform'] as $data_nurse5c) {
				$sid[] = $data_nurse5c['customer_nurse5c_id'];
			}
			
			foreach ($totaldata as $ttdt ) {
				if ( ! in_array($ttdt['customer_nurse5c_id'], $sid) ) {
					$this->db->query("DELETE FROM " . DB_PREFIX . "customer_nurse5c WHERE customer_nurse5c_id = '". (int)$ttdt['customer_nurse5c_id'] ."' AND customer_id = '" . (int)$this->request->get['customer_id'] . "'");
				}
			}
			*/
      		...
      		foreach ($data['nurse5cform'] as $data_nurse5c) {	
				...
				for ($i=1;$i<=26;$i++) {
					if (!isset($data_nurse5c['chk'.$i.''])) { $data_nurse5c['chk'.$i.''] = ''; }
				}
		
		/*
				if (!isset($data_nurse5c['chk27a'])) { $data_nurse5c['chk27a'] = ''; }
				if (!isset($data_nurse5c['chk27b'])) { $data_nurse5c['chk27b'] = ''; }
				if (isset($data_nurse5c['chk37a']) AND $data_nurse5c['chk37a']<>'') { 
					$c37adatetime = $data_nurse5c['date_added']." ".$data_nurse5c['chk37a'];
				} else {
					$data_nurse5c['chk37a'] = ''; $c37adatetime = '';
				}
				if (isset($data_nurse5c['chk37b']) AND $data_nurse5c['chk37b']<>'') { 
					print $c37bdatetime = $data_nurse5c['date_added']." ".$data_nurse5c['chk37b'];
				} else {
					$data_nurse5c['chk37b'] = ''; $c37bdatetime = '';
				}
		...
		*/
				...
				//$data_nurse5c_days_afterbirth = ($data_nurse5c['days_afterbirth'] == '') ? 'NULL' : "'".(int)$data_nurse5c['days_afterbirth']."'"; 
				//$data_nurse5c_days_livein = ($data_nurse5c['days_livein'] == '') ? 'NULL' : "'".(int)$data_nurse5c['days_livein']."'"; 
				//$data_nurse5c_chk24 = ($data_nurse5c['chk24'] == '') ? 'NULL' : "'".(float)$data_nurse5c['chk24']."'"; 
				//$data_nurse5c_chk25 = ($data_nurse5c['chk25'] == '') ? 'NULL' : "'".(int)$data_nurse5c['chk25']."'"; 
				//$data_nurse5c_chk26 = ($data_nurse5c['chk26'] == '') ? 'NULL' : "'".(int)$data_nurse5c['chk26']."'"; 
				//$data_nurse5c_chk27a = ($data_nurse5c['chk27a'] == '') ? 'NULL' : "'".(int)$data_nurse5c['chk27a']."'"; 
				//$data_nurse5c_chk27b = ($data_nurse5c['chk27b'] == '') ? 'NULL' : "'".(int)$data_nurse5c['chk27b']."'"; 
			
				if ($data_nurse5c['customer_nurse5c_id']<>'') {
					$this->db->query("UPDATE " . DB_PREFIX . "customer_nurse5c SET nurse_id = '".(int)$data_nurse5c['nurse_id']."', chk1 = '" . $this->db->escape($data_nurse5c['chk1']) . "', chk2 = '" . $this->db->escape($data_nurse5c['chk2']) . "', chk3 = '" . $this->db->escape($data_nurse5c['chk3']) . "', chk4 = '" . $this->db->escape($data_nurse5c['chk4']) . "', chk5 = '" . $this->db->escape($data_nurse5c['chk5']) . "', chk6 = '" . $this->db->escape($data_nurse5c['chk6']) . "', chk7 = '" . $this->db->escape($data_nurse5c['chk7']) . "', chk8 = '" . $this->db->escape($data_nurse5c['chk8']) . "', chk9 = '" . $this->db->escape($data_nurse5c['chk9']) . "', chk10 = '" . $this->db->escape($data_nurse5c['chk10']) . "', chk11 = '" . $this->db->escape($data_nurse5c['chk11']) . "', chk12 = '" . $this->db->escape($data_nurse5c['chk12']) . "', chk13 = '" . $this->db->escape($data_nurse5c['chk13']) . "', chk14 = '" . $this->db->escape($data_nurse5c['chk14']) . "', chk15 = '" . $this->db->escape($data_nurse5c['chk15']) . "', chk16 = '" . $this->db->escape($data_nurse5c['chk16']) . "', chk17 = '" . $this->db->escape($data_nurse5c['chk17']) . "', chk18 = '" . $this->db->escape($data_nurse5c['chk18']) . "', chk19 = '" . $this->db->escape($data_nurse5c['chk19']) . "', chk20 = '" . $this->db->escape($data_nurse5c['chk20']) . "', chk21 = '" . $this->db->escape($data_nurse5c['chk21']) . "', chk22 = '" . $this->db->escape($data_nurse5c['chk22']) . "', chk23 = '" . $this->db->escape($data_nurse5c['chk23']) . "', chk24= '" . $this->db->escape($data_nurse5c['chk24']) . "', chk25= '" . $this->db->escape($data_nurse5c['chk25']) . "', chk26= '" . $this->db->escape($data_nurse5c['chk26']) . "', date_added = '".$this->db->escape($data_nurse5c['date_added']). "', date_modify = NOW() WHERE customer_nurse5c_id = '". (int)$data_nurse5c['customer_nurse5c_id'] ."' AND customer_id = '" . (int)$this->request->get['customer_id'] . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "customer_nurse5c SET customer_id = '" . (int)$this->request->get['customer_id'] . "', nurse_id = '".(int)$data_nurse5c['nurse_id']."', chk1 = '" . $this->db->escape($data_nurse5c['chk1']) . "', chk2 = '" . $this->db->escape($data_nurse5c['chk2']) . "', chk3 = '" . $this->db->escape($data_nurse5c['chk3']) . "', chk4 = '" . $this->db->escape($data_nurse5c['chk4']) . "', chk5 = '" . $this->db->escape($data_nurse5c['chk5']) . "', chk6 = '" . $this->db->escape($data_nurse5c['chk6']) . "', chk7 = '" . $this->db->escape($data_nurse5c['chk7']) . "', chk8 = '" . $this->db->escape($data_nurse5c['chk8']) . "', chk9 = '" . $this->db->escape($data_nurse5c['chk9']) . "', chk10 = '" . $this->db->escape($data_nurse5c['chk10']) . "', chk11 = '" . $this->db->escape($data_nurse5c['chk11']) . "', chk12 = '" . $this->db->escape($data_nurse5c['chk12']) . "', chk13 = '" . $this->db->escape($data_nurse5c['chk13']) . "', chk14 = '" . $this->db->escape($data_nurse5c['chk14']) . "', chk15 = '" . $this->db->escape($data_nurse5c['chk15']) . "', chk16 = '" . $this->db->escape($data_nurse5c['chk16']) . "', chk17 = '" . $this->db->escape($data_nurse5c['chk17']) . "', chk18 = '" . $this->db->escape($data_nurse5c['chk18']) . "', chk19 = '" . $this->db->escape($data_nurse5c['chk19']) . "', chk20 = '" . $this->db->escape($data_nurse5c['chk20']) . "', chk21 = '" . $this->db->escape($data_nurse5c['chk21']) . "', chk22 = '" . $this->db->escape($data_nurse5c['chk22']) . "', chk23 = '" . $this->db->escape($data_nurse5c['chk23']) . "', chk24= '" . $this->db->escape($data_nurse5c['chk24']) . "', chk25= '" . $this->db->escape($data_nurse5c['chk25']) . "', chk26= '" . $this->db->escape($data_nurse5c['chk26']) . "', date_added = '".$this->db->escape($data_nurse5c['date_added']). "', date_modify = NOW() ");
				}
			}
		}
		
		//$this->db->query("INSERT INTO " . DB_PREFIX . "customer_nurse5c SET customer_id = '".(int)$this->request->get['customer_id']."', nurse_id = '".(int)$data_nurse5c['nurse_id']."',  chk1 = '" . $this->db->escape($data_nurse5c['chk1']) . "', chk2 = '".$this->db->escape($data_nurse5c['chk2'])."', chk3 = '" . $this->db->escape($data_nurse5c['chk3']) . "', chk4 = '" . $this->db->escape($data_nurse5c['chk4']) . "',  chk5 = '".$this->db->escape($data_nurse5c['chk5'])."', chk6 = '".$this->db->escape($data_nurse5c['chk6'])."' , chk7 = '" . $this->db->escape($data_nurse5c['chk7']) . "', chk1_score = '" . (int)$data_nurse5c['chk1_score'] . "', chk2_score = '" . (int)$data_nurse5c['chk2_score'] . "', chk3_score = '" . (int)$data_nurse5c['chk3_score'] . "', chk4_score = '" . (int)$data_nurse5c['chk4_score'] . "', chk5_score = '" . (int)$data_nurse5c['chk5_score'] . "', chk6_score = '" . (int)$data_nurse5c['chk6_score'] . "', chk7_score = '" . (int)$data_nurse5c['chk7_score'] . "',  date_added = NOW() ");
	}
	
	public function editNurse5c($customer_nurse5c_id, $data) {		//其實沒在用
		$totalscore = 0;
		for ($i=1;$i<=7;$i++) {
	...
	public function deleteNurse5c($customer_id,$id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_nurse5c WHERE customer_id = '" . (int)$customer_id . "' AND customer_nurse5c_id = '" . (int)$id . "' ");
	...
	public function getNurse5cs($customer_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_nurse5c WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added");
	
		return $query->rows;
	}
	
	public function getNurse5c($customer_id, $customer_nurse5c_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_nurse5c WHERE customer_id = '" . (int)$customer_id . "' AND customer_nurse5c_id = '". (int)$customer_nurse5c_id ."'");
