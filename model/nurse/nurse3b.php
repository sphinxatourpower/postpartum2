...
class ModelNurseNurse3b extends Model {
			...
			*/
      		...
      		foreach ($data['nurse3bform'] as $k => $data_nurse3b) {	
				...
				*/
				...
				$c18 = (isset($data_nurse3b['chk18']) && is_array($data_nurse3b['chk18'])) ? "'".serialize($data_nurse3b['chk18'])."'" : 'NULL';
				$c19supplement = ($data_nurse3b['chk19']=='已注射' && isset($data_nurse3b['chk19supplement']) && is_array($data_nurse3b['chk19supplement'])) ? "'". serialize($data_nurse3b['chk19supplement']) ."'" : 'NULL';
				$c20supplement = (isset($data_nurse3b['chk20supplement']) && is_array($data_nurse3b['chk20supplement'])) ? "'". serialize($data_nurse3b['chk20supplement']) ."'" : 'NULL';
				$c23supplement = (isset($data_nurse3b['chk23supplement']) && is_array($data_nurse3b['chk23supplement'])) ? "'". serialize($data_nurse3b['chk23supplement']) ."'" : 'NULL';
	...
	*/
	...
	public function getLastTemperature($customer_child_id) {
		...
		$query = $this->db->query("SELECT chk1  FROM " . DB_PREFIX . "child_nurse3b WHERE customer_child_id = '" . (int)$customer_child_id . "' AND date_added = '". $this->db->escape($pdate) ."' AND arrangement = '". $this->db->escape($parra) ."' ");
		
		if ($query->num_rows) {
			return $query->row['chk1'];
		} else {
			return false;
		}
	}
	
	public function getLastWeight($customer_child_id) {
		...
		$query = $this->db->query("SELECT chk10  FROM " . DB_PREFIX . "child_nurse3b WHERE customer_child_id = '" . (int)$customer_child_id . "' AND date_added = '". $this->db->escape($pdate) ."' AND arrangement = '". $this->db->escape($parra) ."' ");
		
		if ($query->num_rows) {
			return $query->row['chk10'];
		} else {
			return false;
		}
	}
	
	public function getLastJaundice($customer_child_id) {
		...
		$query = $this->db->query("SELECT chk13  FROM " . DB_PREFIX . "child_nurse3b WHERE customer_child_id = '" . (int)$customer_child_id . "' AND date_added = '". $this->db->escape($pdate) ."' AND arrangement = '". $this->db->escape($parra) ."' ");
		
		if ($query->num_rows) {
			return $query->row['chk13'];
		} else {
			return false;
		}
	}
	
	public function getLastPiss($customer_child_id) {
		...
		$query = $this->db->query("SELECT chk11  FROM " . DB_PREFIX . "child_nurse3b WHERE customer_child_id = '" . (int)$customer_child_id . "' AND date_added = subdate(current_date, 1) ");
		
		if ($query->num_rows) {
			return $query->row['chk11'];
		} else {
			return false;
		}
	}
