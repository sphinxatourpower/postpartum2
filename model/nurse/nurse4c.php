<?php
class ModelNurseNurse4c extends Model {
	public function addNurse4c($data) {		
		for ($i=1;$i<=10;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
		}
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer_nurse4c SET customer_id = '".(int)$this->request->get['customer_id']."', score = '".(int)$data['score']."', chk1 = '" . $this->db->escape($data['chk1']) . "', chk2 = '".$this->db->escape($data['chk2'])."', chk3 = '" . $this->db->escape($data['chk3']) . "', chk4 = '" . $this->db->escape($data['chk4']) . "',  chk5 = '".$this->db->escape($data['chk5'])."', chk6 = '".$this->db->escape($data['chk6'])."' , chk7 = '" . $this->db->escape($data['chk7']) . "', chk8 = '" . $this->db->escape($data['chk8']) . "', chk9 = '" . $this->db->escape($data['chk9']) . "', chk10 = '".$this->db->escape($data['chk10']) ."', date_added = NOW() ");
	}
	
	public function editNurse4c($customer_nurse4c_id, $data) {		
		for ($i=1;$i<=10;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
		}
		$this->db->query("UPDATE " . DB_PREFIX . "customer_nurse4c SET score = '".(int)$data['score']."', chk1 = '" . $this->db->escape($data['chk1']) . "', chk2 = '".$this->db->escape($data['chk2'])."', chk3 = '" . $this->db->escape($data['chk3']) . "', chk4 = '" . $this->db->escape($data['chk4']) . "',  chk5 = '".$this->db->escape($data['chk5'])."', chk6 = '".$this->db->escape($data['chk6'])."' , chk7 = '" . $this->db->escape($data['chk7']) . "', chk8 = '" . $this->db->escape($data['chk8']) . "', chk9 = '" . $this->db->escape($data['chk9']) . "', chk10 = '".$this->db->escape($data['chk10']) ."', date_modify = NOW() WHERE customer_nurse4c_id = '".(int)$customer_nurse4c_id."'");
	}
	...
	public function getNurse4c($customer_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_nurse4c WHERE customer_id = '" . (int)$customer_id . "'");
