<?php
class ModelNurseBeck extends Model {
	public function addNurse4c($data) {		
		for ($i=1;$i<=10;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
		}
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer_beck SET customer_id = '".(int)$this->request->get['customer_id']."', score = '".(int)$data['score']."', chk1 = '" . $this->db->escape($data['chk1']) . "', chk2 = '".$this->db->escape($data['chk2'])."', chk3 = '" . $this->db->escape($data['chk3']) . "', chk4 = '" . $this->db->escape($data['chk4']) . "',  chk5 = '".$this->db->escape($data['chk5'])."', chk6 = '".$this->db->escape($data['chk6'])."' , chk7 = '" . $this->db->escape($data['chk7']) . "', chk8 = '" . $this->db->escape($data['chk8']) . "', chk9 = '" . $this->db->escape($data['chk9']) . "', chk10 = '".$this->db->escape($data['chk10']) ."', chk11 = '".$this->db->escape($data['chk11']) ."', chk12 = '".$this->db->escape($data['chk12']) ."', chk13 = '".$this->db->escape($data['chk13']) ."', chk14 = '".$this->db->escape($data['chk14']) ."', chk15 = '".$this->db->escape($data['chk15']) ."', chk16 = '".$this->db->escape($data['chk16']) ."', chk17 = '".$this->db->escape($data['chk17']) ."', chk18 = '".$this->db->escape($data['chk18']) ."', chk19 = '".$this->db->escape($data['chk19']) ."', chk20 = '".$this->db->escape($data['chk20']) ."', chk21 = '".$this->db->escape($data['chk21']) ."', date_added = NOW() ");
	}
	
	public function editNurse4c($customer_beck_id, $data) {		
		for ($i=1;$i<=10;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
		}
		$this->db->query("UPDATE " . DB_PREFIX . "customer_beck SET score = '".(int)$data['score']."', chk1 = '" . $this->db->escape($data['chk1']) . "', chk2 = '".$this->db->escape($data['chk2'])."', chk3 = '" . $this->db->escape($data['chk3']) . "', chk4 = '" . $this->db->escape($data['chk4']) . "',  chk5 = '".$this->db->escape($data['chk5'])."', chk6 = '".$this->db->escape($data['chk6'])."' , chk7 = '" . $this->db->escape($data['chk7']) . "', chk8 = '" . $this->db->escape($data['chk8']) . "', chk9 = '" . $this->db->escape($data['chk9']) . "', chk10 = '".$this->db->escape($data['chk10']) ."', chk11 = '".$this->db->escape($data['chk11']) ."', chk12 = '".$this->db->escape($data['chk12']) ."', chk13 = '".$this->db->escape($data['chk13']) ."', chk14 = '".$this->db->escape($data['chk14']) ."', chk15 = '".$this->db->escape($data['chk15']) ."', chk16 = '".$this->db->escape($data['chk16']) ."', chk17 = '".$this->db->escape($data['chk17']) ."', chk18 = '".$this->db->escape($data['chk18']) ."', chk19 = '".$this->db->escape($data['chk19']) ."', chk20 = '".$this->db->escape($data['chk20']) ."', chk21 = '".$this->db->escape($data['chk21']) ."', date_modify = NOW() WHERE customer_beck_id = '".(int)$customer_beck_id."'");
	}
	...
	public function getNurse4c($customer_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_beck WHERE customer_id = '" . (int)$customer_id . "'");
