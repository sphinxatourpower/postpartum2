<?php
class ModelNurseNurse3c extends Model {
	public function addNurse3c($data) {		
		$totalscore = 0;
		for ($i=1;$i<=8;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
			if (!isset($data['chk'.$i.'_score'])) { $data['chk'.$i.'_score'] = 0; }
			$totalscore += (int)$data['chk'.$i.'_score'];
		}
		
		$score_total = ($data['score_total']=='' OR $data['score_total']!=$totalscore) ? $totalscore  : $data['score_total']  ;
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer_nurse3c SET customer_id = '".(int)$this->request->get['customer_id']."', nurse_id = '".(int)$data['nurse_id']."', score_total = '".(int)$score_total."', chk1 = '" . $this->db->escape($data['chk1']) . "', chk2 = '".$this->db->escape($data['chk2'])."', chk3 = '" . $this->db->escape($data['chk3']) . "', chk4 = '" . $this->db->escape($data['chk4']) . "',  chk5 = '".$this->db->escape($data['chk5'])."', chk6 = '".$this->db->escape($data['chk6'])."' , chk7 = '" . $this->db->escape($data['chk7']) . "', chk8 = '" . $this->db->escape($data['chk8']) . "', chk1_score = '" . (int)$data['chk1_score'] . "', chk2_score = '" . (int)$data['chk2_score'] . "', chk3_score = '" . (int)$data['chk3_score'] . "', chk4_score = '" . (int)$data['chk4_score'] . "', chk5_score = '" . (int)$data['chk5_score'] . "', chk6_score = '" . (int)$data['chk6_score'] . "', chk7_score = '" . (int)$data['chk7_score'] . "', chk8_score = '" . (int)$data['chk8_score'] . "', date_added = NOW() ");
	}
	
	public function editNurse3c($customer_nurse3c_id, $data) {		
		$totalscore = 0;
		for ($i=1;$i<=8;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
			if (!isset($data['chk'.$i.'_score'])) { $data['chk'.$i.'_score'] = 0; }
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer_nurse3c SET nurse_id = '".(int)$data['nurse_id']."', score_total = '".(int)$score_total."', chk1 = '" . $this->db->escape($data['chk1']) . "', chk2 = '".$this->db->escape($data['chk2'])."', chk3 = '" . $this->db->escape($data['chk3']) . "', chk4 = '" . $this->db->escape($data['chk4']) . "',  chk5 = '".$this->db->escape($data['chk5'])."', chk6 = '".$this->db->escape($data['chk6'])."' , chk7 = '" . $this->db->escape($data['chk7']) . "', chk8 = '" . $this->db->escape($data['chk8']) . "', chk1_score = '" . (int)$data['chk1_score'] . "', chk2_score = '" . (int)$data['chk2_score'] . "', chk3_score = '" . (int)$data['chk3_score'] . "', chk4_score = '" . (int)$data['chk4_score'] . "', chk5_score = '" . (int)$data['chk5_score'] . "', chk6_score = '" . (int)$data['chk6_score'] . "', chk7_score = '" . (int)$data['chk7_score'] . "', chk8_score = '" . (int)$data['chk8_score'] . "', date_modify = NOW() WHERE customer_nurse3c_id = '".(int)$customer_nurse3c_id."'");
	}
	...
	public function deleteNurse3c($customer_nurse3c_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_nurse3c WHERE customer_nurse3c_id = '" . (int)$customer_nurse3c_id . "'");
	...
	public function getNurse3cs($customer_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_nurse3c WHERE customer_id = '" . (int)$customer_id . "'");
	
		return $query->rows;
	}
	
	public function getNurse3c($customer_id, $customer_nurse3c_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_nurse3c WHERE customer_id = '" . (int)$customer_id . "' AND customer_nurse3c_id = '". (int)$customer_nurse3c_id ."'");
