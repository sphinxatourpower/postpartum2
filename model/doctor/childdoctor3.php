<?php
class ModelDoctorChildDoctor3 extends Model {
	public function addCustomerDoctor3($data) {
		//$this->db->query("DELETE FROM " . DB_PREFIX . "customer_child_doctor3 WHERE customer_id = '" . (int)$this->request->get['customer_id'] . "'");
		
		//print"<pre>";print_r($data['childdoctor3form']);print"</pre>";exit;
		
		if (isset($data['childdoctor3form'])) {	
			foreach ($data['childdoctor3form'] as $data_childdoctor3) {	
				for ($i=1;$i<=10;$i++) {
					if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
				}
				
				if ($data_childdoctor3['customer_child_doctor3_id']<>'') {
					$this->db->query("UPDATE " . DB_PREFIX . "customer_child_doctor3 SET doctor_id = '".(int)$data_childdoctor3['doctor_id']."', chk1 = '" . $this->db->escape($data_childdoctor3['chk1']) . "', chk2 = '".$this->db->escape($data_childdoctor3['chk2'])."', chk3 = '" . $this->db->escape($data_childdoctor3['chk3']) . "', chk4 = '".$this->db->escape($data_childdoctor3['chk4'])."', chk5 = '" . $this->db->escape($data_childdoctor3['chk5']) . "', chk6 = '".$this->db->escape((isset($data_childdoctor3['chk6']) ? serialize($data_childdoctor3['chk6']) : ''))."' ,  chk7 = '" . $this->db->escape((isset($data_childdoctor3['chk7']) ? serialize($data_childdoctor3['chk7']) : '')) . "', chk8 = '".$this->db->escape((isset($data_childdoctor3['chk8']) ? serialize($data_childdoctor3['chk8']) : ''))."', chk9 = '".$this->db->escape((isset($data_childdoctor3['chk9']) ? serialize($data_childdoctor3['chk9']) : ''))."', chk10 = '" . $this->db->escape($data_childdoctor3['chk10']) . "', supplement = '" . $this->db->escape($data_childdoctor3['supplement']) . "', date_added = '". $this->db->escape($data_childdoctor3['date_added']). "' WHERE customer_child_doctor3_id = '".(int)$data_childdoctor3['customer_child_doctor3_id']."' AND customer_id = '". (int)$this->request->get['customer_child_id'] ."'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "customer_child_doctor3 SET customer_id = '".(int)$this->request->get['customer_id']."', doctor_id = '".(int)$data_childdoctor3['doctor_id']."', chk1 = '" . $this->db->escape($data_childdoctor3['chk1']) . "', chk2 = '".$this->db->escape($data_childdoctor3['chk2'])."', chk3 = '" . $this->db->escape($data_childdoctor3['chk3']) . "', chk4 = '".$this->db->escape($data_childdoctor3['chk4'])."', chk5 = '" . $this->db->escape($data_childdoctor3['chk5']) . "', chk6 = '".$this->db->escape((isset($data_childdoctor3['chk6']) ? serialize($data_childdoctor3['chk6']) : ''))."' ,  chk7 = '" . $this->db->escape((isset($data_childdoctor3['chk7']) ? serialize($data_childdoctor3['chk7']) : '')) . "', chk8 = '".$this->db->escape((isset($data_childdoctor3['chk8']) ? serialize($data_childdoctor3['chk8']) : ''))."', chk9 = '".$this->db->escape((isset($data_childdoctor3['chk9']) ? serialize($data_childdoctor3['chk9']) : ''))."', chk10 = '" . $this->db->escape($data_childdoctor3['chk10']) . "',  supplement = '" . $this->db->escape($data_childdoctor3['supplement']) . "', date_added = '". $this->db->escape($data_childdoctor3['date_added']). "', date_modify = NOW() ");
				}
			}
		}
	}
	
	public function editCustomerDoctor3($customer_child_doctor3_id, $data) {		//其實沒在用
		for ($i=1;$i<=33;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
		}
	...
	public function deleteChildDoctor3($customer_id,$id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_child_doctor3 WHERE customer_id = '" . (int)$customer_id . "' AND customer_child_doctor3_id = '" . (int)$id . "' ");
	...
	public function getCustomerDoctor3($customer_id, $doctor3id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_child_doctor3 WHERE customer_id = '" . (int)$customer_id . "' AND customer_child_doctor3_id = '".  (int)$doctor3id ."' ");
	
		return $query->row;
	}
	
	public function getCustomerDoctor3s($customer_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_child_doctor3 WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added");
