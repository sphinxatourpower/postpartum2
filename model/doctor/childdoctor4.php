<?php
class ModelDoctorChildDoctor4 extends Model {
	public function addCustomerDoctor4($data) {
		//$this->db->query("DELETE FROM " . DB_PREFIX . "customer_child_doctor4 WHERE customer_id = '" . (int)$this->request->get['customer_id'] . "'");
		
		//print"<pre>";print_r($data['childdoctor4form']);print"</pre>";exit;
		
		if (isset($data['childdoctor4form'])) {	
			foreach ($data['childdoctor4form'] as $data_childdoctor4) {	
				for ($i=1;$i<=10;$i++) {
					if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
				}
				
				if ($data_childdoctor4['customer_child_doctor4_id']<>'') {
					$this->db->query("UPDATE " . DB_PREFIX . "customer_child_doctor4 SET doctor_id = '".(int)$data_childdoctor4['doctor_id']."', chk1 = '" . $this->db->escape($data_childdoctor4['chk1']) . "', chk2 = '".$this->db->escape($data_childdoctor4['chk2'])."', chk3 = '" . $this->db->escape($data_childdoctor4['chk3']) . "', chk4 = '".$this->db->escape($data_childdoctor4['chk4'])."', chk5 = '" . $this->db->escape($data_childdoctor4['chk5']) . "', chk6 = '".$this->db->escape((isset($data_childdoctor4['chk6']) ? serialize($data_childdoctor4['chk6']) : ''))."' ,  chk7 = '" . $this->db->escape((isset($data_childdoctor4['chk7']) ? serialize($data_childdoctor4['chk7']) : '')) . "', chk8 = '".$this->db->escape((isset($data_childdoctor4['chk8']) ? serialize($data_childdoctor4['chk8']) : ''))."', chk9 = '".$this->db->escape((isset($data_childdoctor4['chk9']) ? serialize($data_childdoctor4['chk9']) : ''))."', chk10 = '" . $this->db->escape($data_childdoctor4['chk10']) . "', supplement = '" . $this->db->escape($data_childdoctor4['supplement']) . "', date_added = '". $this->db->escape($data_childdoctor4['date_added']). "' WHERE _id = '".(int)$data_childdoctor4['customer_child_doctor4_id']."' AND customer_id = '". (int)$this->request->get['customer_child_id'] ."'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "customer_child_doctor4 SET customer_id = '".(int)$this->request->get['customer_id']."', doctor_id = '".(int)$data_childdoctor4['doctor_id']."', chk1 = '" . $this->db->escape($data_childdoctor4['chk1']) . "', chk2 = '".$this->db->escape($data_childdoctor4['chk2'])."', chk3 = '" . $this->db->escape($data_childdoctor4['chk3']) . "', chk4 = '".$this->db->escape($data_childdoctor4['chk4'])."', chk5 = '" . $this->db->escape($data_childdoctor4['chk5']) . "', chk6 = '".$this->db->escape((isset($data_childdoctor4['chk6']) ? serialize($data_childdoctor4['chk6']) : ''))."' ,  chk7 = '" . $this->db->escape((isset($data_childdoctor4['chk7']) ? serialize($data_childdoctor4['chk7']) : '')) . "', chk8 = '".$this->db->escape((isset($data_childdoctor4['chk8']) ? serialize($data_childdoctor4['chk8']) : ''))."', chk9 = '".$this->db->escape((isset($data_childdoctor4['chk9']) ? serialize($data_childdoctor4['chk9']) : ''))."', chk10 = '" . $this->db->escape($data_childdoctor4['chk10']) . "',  supplement = '" . $this->db->escape($data_childdoctor4['supplement']) . "', date_added = '". $this->db->escape($data_childdoctor4['date_added']). "', date_modify = NOW() ");
				}
			}
		}
	}
	
	public function editCustomerDoctor4($customer_child_doctor4_id, $data) {		//其實沒在用
		for ($i=1;$i<=33;$i++) {
			if (!isset($data['chk'.$i.''])) { $data['chk'.$i.''] = ''; }
		}
	...
	public function deleteChildDoctor4($customer_id,$id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_child_doctor4 WHERE customer_id = '" . (int)$customer_id . "' AND customer_child_doctor4_id = '" . (int)$id . "' ");
	...
	public function getCustomerDoctor4($customer_id, $doctor4id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_child_doctor4 WHERE customer_id = '" . (int)$customer_id . "' AND customer_child_doctor4_id = '".  (int)$doctor4id ."' ");
	
		return $query->row;
	}
	
	public function getCustomerDoctor4s($customer_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_child_doctor4 WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added");
