...
class ModelDoctorChildDoctor2 extends Model {
	public function addChildDoctor2($data, $union_id = 0) {
		...
		if (isset($data['childdoctor2form'])) {	
			foreach ($data['childdoctor2form'] as $data_childdoctor2) {	
				if (!$union_id) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "customer_child_doctor2_union SET date_added = '". $this->db->escape($data_childdoctor2['date_added']). "', date_modify = NOW(), doctor_id = '".(int)$data_childdoctor2['doctor_id']."' ");
					
					$union_id = $this->db->getLastId();
				} else {
					$this->db->query("UPDATE " . DB_PREFIX . "customer_child_doctor2_union SET date_added = '". $this->db->escape($data_childdoctor2['date_added']). "', date_modify = NOW(), doctor_id = '".(int)$data_childdoctor2['doctor_id']."' WHERE union_id = '". (int)$union_id ."' ");
				}
			
				$nar = array(4,5,8,9,11,12,13,14,15,16,17,18,19,21,22,23,26,27,28,29,30,31,32,33,34);
				for ($i=1;$i<=35;$i++) {
					if (!isset($data_childdoctor2['chk'.$i.''])) { 
						if (in_array($i, $nar)) {
							$data_childdoctor2['chk'.$i.''] = array();
						} else {
							$data_childdoctor2['chk'.$i.''] = '';
						}
					}
				}
				
				if (!isset($data_childdoctor2['groupchk'])) { $data_childdoctor2['groupchk'] = array(); }
				if (!isset($data_childdoctor2['prescription'])) { $data_childdoctor2['prescription'] = ''; }
				if (!isset($data_childdoctor2['medicine'])) { $data_childdoctor2['medicine'] = array(); }
				
							$medicine = array();
							if (!empty($data_childdoctor2['medicine'])) {
								foreach ($data_childdoctor2['medicine'] as $k => $ccm) {
									if ($ccm == "其他") {
										$medicine[$k] = $ccm.":".$data_childdoctor2['medicine_other'];
									} else {
										$medicine[$k] = $ccm;
									}
								}
	...
	public function deleteChildDoctor2($union_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_child_doctor2_union WHERE union_id = '" . (int)$union_id . "' ");
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_child_doctor2 WHERE union_id = '" . (int)$union_id . "' ");
	...
	*/
	...
	public function getChildDoctor2($customer_child_id, $union_id) {
		$query = $this->db->query("SELECT cd2.*, cc.name AS child_name, cc.gender AS child_gender, u.date_added AS date_added, u.doctor_id FROM " . DB_PREFIX . "customer_child_doctor2 cd2 
		LEFT JOIN ". DB_PREFIX . "customer_child cc ON (cc.customer_child_id = cd2.customer_child_id) 
		LEFT JOIN ". DB_PREFIX . "customer_child_doctor2_union u ON (u.union_id = cd2.union_id) 
		WHERE cd2.union_id = '". (int)$union_id ."' AND cd2.customer_child_id = '". (int)$customer_child_id ."' ");
	
		return $query->row;
	}
	
	public function getChildDoctor2Unions($data = array()) {
		$sql = "SELECT u.*, uu.name AS name FROM " . DB_PREFIX . "customer_child_doctor2_union u LEFT JOIN ". DB_PREFIX . "user uu ON (u.doctor_id = uu.user_id) ";
		
		$implode = array();
		
		if (!empty($data['filter_name'])) {
			$implode[] = "LCASE(uu.name) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
		}
		
		if (!empty($data['filter_date_added'])) {
			$implode[] = "DATE(u.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}
	...
	public function getChildDoctor2Union($union_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_child_doctor2_union WHERE union_id = '" . (int)$union_id . "' ");
	
		return $query->row;
	}
	
	public function getChildDoctor2UnionDatas($union_id) {
		$query = $this->db->query("SELECT cd2.*, cc.name AS child_name, cc.gender AS child_gender, u.date_added AS date_added, u.doctor_id FROM " . DB_PREFIX . "customer_child_doctor2 cd2 
		LEFT JOIN ". DB_PREFIX . "customer_child cc ON (cc.customer_child_id = cd2.customer_child_id) 
		LEFT JOIN ". DB_PREFIX . "customer_child_doctor2_union u ON (u.union_id = cd2.union_id) 
		JOIN ". DB_PREFIX . "customer c ON (c.customer_id = cc.customer_id) 
		WHERE cd2.union_id = '". (int)$union_id ."' ORDER BY cc.bed_id, cd2.customer_child_id ");
	
		return $query->rows;
	}
	
	public function getTotalChildDoctor2Unions($data = array()) {
		...
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer_child_doctor2_union u LEFT JOIN ". DB_PREFIX . "user uu ON (u.doctor_id = uu.user_id) ";
