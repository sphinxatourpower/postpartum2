...
class ModelServiceCustomer extends Model {
	public function addCustomer($data, $customer_room_id = 0) {
		...
		if ($data['sick_history']=='0'){
			$data['sick_history'] = $data['sick_history'];
		}elseif ($data['sick_history']=='1'){ 
			$data['sick_history'] = $data['sick_history'].":".$data['sick_history_text']; 
		}else{
			$data['sick_history'] = '';
		}
		...
		if (!empty($data['complications'])) {
			foreach ($data['complications'] as $string) {
				if ($string == "感染") {
					$complications[] .= $string.":".$data['complications_other1'];
				} elseif($string == "產道撕裂傷"){
					$complications[] .= $string.":".$data['complications_other2'];
				}else{
		...
		$food = array();
		if (!empty($data['food'])) {
			foreach ($data['food'] as $string) {
				if ($string == "其他") {
					$food[] .= $string.":".$data['food_other'];
				} else {
					$food[] .= $string;
				}
			}
		...
		$is_foreigner = (!isset($data['foreigner']) || $data['foreigner']=='') ? 0 : 1;
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET idnumber = UPPER('".$this->db->escape($data['idnumber'])."'), foreigner = '" . (int)$is_foreigner . "', name = '" . $this->db->escape($data['name']) . "', birthdate = '" . $this->db->escape($data['birthdate']) . "', address= '" . $this->db->escape($data['address']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', email = '" . $this->db->escape($data['email']) . "',  fall_score = ". $data_fall_score.", depression_score = ". $data_depression_score." , degree = '" . $this->db->escape($data['degree']) . "', profession = '" . $this->db->escape($pro) . "', language = '" . $this->db->escape($lng) . "', sick_history='" . $this->db->escape($data['sick_history']) . "', memo='" . $this->db->escape($data['memo']) . "', complications='".serialize($complications)."', food='".serialize($food)."', evacuation_hospital = '".$this->db->escape($evh) ."', status = '" . (int)$data['status'] . "', showathome = '1', service_id = '" . (int)$data['service_id'] . "', date_leave = '" . $this->db->escape($data['date_leave']." ".$data['date_leave_time']) . "', date_added = '" . $this->db->escape($data['date_added']." ".$data['date_added_time']) . "', spouse_name = '" . $this->db->escape($data['spouse_name']) . "', childbirth_hospital = '".$this->db->escape($cbh)."', childbirth_doctor = '" . $this->db->escape($data['childbirth_doctor']) . "', childbirth_method = '" . $this->db->escape($data['childbirth_method']) . "', child_count_g = " .  $data_child_count_g . ", child_count_p = " .  $data_child_count_p . ", pregnancy_weeks = " .  $data_pregnancy_weeks . ", weeks_plus = " .  $data_weeks_plus . " ");
		...
		if ($customer_room_id) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_transfer SET customer_id = '" . (int)$customer_id . "', customer_room_id = '" . (int)$customer_room_id . "',  date_added = NOW(), description = '新入住', extra_period = '0' ");
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_room_id = '".(int)$customer_room_id."' WHERE customer_id = '". (int)$customer_id ."'"); 
		}
	}
