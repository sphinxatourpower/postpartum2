...
class ModelToolWorddoc extends Model {
	var $appName = APPNAME;
	 ...
	 */
	function clsMsDocGenerator($pageOrientation = 'PORTRAIT', $pageType = 'A4', $cssFile = '', $topMargin = 2, $rightMargin = 2, $bottomMargin = 2, $leftMargin = 2){
		$this->documentBuffer = '';
		$this->formatBuffer = '';
	...
	function output($fileName = '', $saveInPath = ''){
		...
		$outputCode .= "	<head><title>Time</title>\n";
		$outputCode .= "		<xml>\n";
		$outputCode .= "			<w:WordDocument>\n";
		$outputCode .= "			<w:View>Print</w:View>\n";
		$outputCode .= "			<w:Zoom>100</w:Zoom>\n";
		$outputCode .= "			</w:WordDocument>\n";
		$outputCode .= "		</xml>\n";
		...
		if($saveInPath == ''){
			if($this->isDebugging){
				...
				if ( preg_match( "/MSIE/", $_SERVER["HTTP_USER_AGENT"] ) )
					header("Content-Disposition: attachment; filename=\"".mb_convert_encoding($fileName, "BIG5", "UTF-8")."\"");
				else
					header("Content-Disposition: attachment; filename=\"$fileName\"");
			...
			file_put_contents(trim($saveInPath . $fileName), $outputCode);
	 ...
	 */
	function bufferImage($imagePath, $width=0, $height=0, $title = ''){
		$this->numImages++;
		$buffer = "<!--[if gte vml 1]>";
		if($this->numImages == 1){
		...
		if (!$width || !$height ) {
			$stylewidth = ($width)? "width:".$this->pixelsToPoints."pt; " : "";
			$styleheight = ($height)? "height:".$this->pixelsToPoints."pt; " : "";
			$stylestr = "style=\"".$stylewidth.$styleheight."\"";
			$widthstr = ($width) ? "width=\"$width\" " : " " ;
			$heightstr = ($height) ? "height=\"$height\" " : " " ;

			$buffer .= "<v:shape id=\"_x0000_i102{$this->numImages}\" type=\"#_x0000_t75\" ".$stylestr.">
			   <v:imagedata src=\"$imagePath\" o:title=\"accessibilityIssues\"/>
			  </v:shape><![endif]--><![if !vml]><img ".$widthstr.$heightstr." src=\"$imagePath\" v:shapes=\"_x0000_i102{$this->numImages}\"><![endif]>";
		} else {		
	...
	function startTable($inlineStyle = NULL, $className = 'normalTable'){
		...
		$this->documentBuffer .= "<div align=\"center\">\n";
	...
	function addTableRow($cells, $aligns = NULL, $vAligns = NULL, $inlineStyle = NULL, $classesName = NULL ){
		...
		if(is_array($classesName) && count($classesName) != count($cells['content']))
			die('ERROR: COUNT OF CLASSES IS DIFERENT OF COUNT OF CELLS');
		if(is_array($aligns) && count($aligns) != count($cells['content']))
			die('ERROR: COUNT OF ALIGNS IS DIFERENT OF COUNT OF CELLS');
		if(is_array($vAligns) && count($vAligns) != count($cells['content']))
		...
		for($i = 0; $i < count($cells['content']); $i++){
			...
			$colspan = (!empty($cells['span'][$i]) AND $cells['span'][$i]<>'') ? 'colspan="'.$cells['span'][$i].'"' : '';
			$rowspan = (!empty($cells['rspan'][$i]) AND $cells['rspan'][$i]<>'') ? 'rowspan="'.$cells['rspan'][$i].'"' : '';
			$cellWidth = (!empty($cells['cellwidth'][$i]) AND $cells['cellwidth'][$i]<>'') ? 'width="'.$cells['cellwidth'][$i].'"' : '';
			$classAttr = is_array($classesName) ? " class=\"$classesName[$i]\"" : '';
			
			$this->documentBuffer .= "<td ".$cellWidth." align=\"".$align."\" valign=\"".$vAlign."\" ".$colspan." ".$rowspan." style=\"$style\" {$classAttr}>".$cells['content'][$i]."</td>\n";
		}
	...
	function prepareDefaultHeader(){	
		...
		$this->formatBuffer .= "   padding: 1.5pt 5.4pt 1.5pt 5.4pt;\n";
		$this->formatBuffer .= "}\n\n";
		...
		$this->formatBuffer .= "   mso-yfti-tbllook: 480;\n";
		$this->formatBuffer .= "   mso-padding-alt: 0.2cm {$this->tablePaddingAltRight}pt 0.2cm {$this->tablePaddingAltLeft}pt;\n";
		$this->formatBuffer .= "   mso-border-insideh: {$this->tableBorderInsideH}pt solid #ccc;\n";
		$this->formatBuffer .= "   mso-border-insidev: {$this->tableBorderInsideV}pt solid #ccc;\n";
		$this->formatBuffer .= "   mso-para-margin: 0cm;\n";
		$this->formatBuffer .= "   mso-para-margin-bottom: .0001pt;\n";
		$this->formatBuffer .= "   mso-pagination: widow-orphan;\n";
		$this->formatBuffer .= "   font-size: {$this->fontSize}pt;\n";
		...
		$this->formatBuffer .= "td.large{\n";
		$this->formatBuffer .= "   font-size: 21pt;\n";
		$this->formatBuffer .= "   font-weight: bold;\n";
		$this->formatBuffer .= "   vertical-align: middle;\n";
		$this->formatBuffer .= "   padding: 0pt 0pt 0pt 0pt;\n";
		$this->formatBuffer .= "}\n\n";
		
		$this->formatBuffer .= "td.med{\n";
		$this->formatBuffer .= "   font-size: 13pt;\n";
		$this->formatBuffer .= "   vertical-align: middle;\n";
		$this->formatBuffer .= "   padding: 0pt 0pt 0pt 0pt;\n";
		$this->formatBuffer .= "}\n\n";
	...
	function getHeader(){
		...
		$header .= "<meta name=\"Generator\" content=\"$this->appName v$this->appVersion\">\n";
		$header .= "<meta name=\"Originator\" content=\"$this->appName v$this->appVersion\">\n";
		$header .= "<!--[if !mso]>\n";
		$header .= "<style>\n";
	 ...
	 * @param $preview: boolean if debug is set
	 */
