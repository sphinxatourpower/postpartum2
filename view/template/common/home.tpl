...
<div id="content">
  ...
  <div class="box">
	...
	<div class="roomarea">
		<?php if (!empty($rooms)) { ?>
		<?php foreach ($rooms as $roo) { ?>
			<h1><?php echo $roo['floor']; ?></h1>
		<?php foreach ($roo['items'] as $room) { ?>
			<div class="homebox">
			  <span class="label"><?php echo $room['name']; ?></span>
			  <p>
			  <?php if ($room['status_val']) { ?>
				<ul>
					<li>媽媽姓名：<a href="<?php echo $room['momdashboard']; ?>"><?php echo $room['customer_name']; ?></a></li>
					<?php if (!empty($room['childs'])) { ?>
					<?php foreach ($room['childs'] as $child) { ?>
					<li>寶寶姓名：<a href="<?php echo $child['babydashboard']; ?>"><?php echo $child['name']; ?></a></li>
					<?php } ?>
					<?php } ?>
				</ul>
			  <?php } ?>
			  </p>
			  <span class="endlabel <?php echo $room['status_class']; ?>"><?php echo ($room['status_val']) ? $room['status'] : '<a href="'.$room['booking'].'">' . $room['status'] . '</a>'; ?></span>
			</div>
		<?php } ?>
...
-->			
	  ...
	  <div id="calendar" style="float:right;width:50%;display:none;"></div>
  </div>
</div>
</div>
