        ...
        <table class="list">
          <thead>
			...
			<td class="left"><?php if ($sort == 'service') { ?>
                ...
                <?php } ?></td>
              <td class="right" colspan="2"><?php echo $column_action; ?></td>
            </tr>
          </thead>
          <tbody>
            <tr class="filter">
              ...
              <td align="right" colspan="2"><a onclick="filter();" class="button"><?php echo $button_filter; ?></a> <a onclick="reset_filter();" class="button"><?php echo $button_reset_filter; ?></a></td>
            </tr>
            <?php if ($customers) { ?>
            ...
            <tr>
              ...
              <td class="center"><?php $ac = array_chunk($customer['action'],2); ?>
				<?php foreach ($ac as $k => $v) {
						$cols = array();
						foreach ($v as $i => $m) {
				?>
				[ <a href="<?php echo $m['href']; ?>"><?php echo $m['text']; ?></a> ]
				<?php	}	?>
				<br />
				<?php } ?></td>
				<td class="right"><?php foreach ($customer['actionadd'] as $actionadd) { ?>
                [ <a href="<?php echo $actionadd['href']; ?>"><?php echo $actionadd['text']; ?></a> ]
                <?php } ?></td>
            </tr>
            <?php } ?>
            <?php } else { ?>
...
function reset_filter() {
	url = 'index.php?route=nurse/nurse3c&token=<?php echo $token; ?>';
	location = url;
}
function filter() {
	url = 'index.php?route=nurse/nurse3c&token=<?php echo $token; ?>';
