      ...
      <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="list">
          <thead>
            <tr>
              ...
              <td class="left"><?php if ($sort == 'name') { ?>
                <a href="<?php echo $sort_date_written; ?>" class="<?php echo strtolower($order); ?>"><?php echo $column_date_written; ?></a>
                <?php } else { ?>
                <a href="<?php echo $sort_date_written; ?>"><?php echo $column_date_written; ?></a>
                <?php } ?></td>
              <td class="left"><?php if ($sort == 'name') { ?>
          ...
          <tbody>
            <tr class="filter">
              <td></td>
			  <td><input type="text" name="filter_date_written" value="<?php echo $filter_date_written; ?>" size="15" class="date" /></td>
            ...
            <tr>
              <td style="text-align: center;"><?php if ($customer['selected']) { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $customer['customer_satisfaction_id']; ?>" checked="checked" />
                <?php } else { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $customer['customer_satisfaction_id']; ?>" />
                <?php } ?></td>
			  <td class="left"><?php echo $customer['date_written']; ?></td>
              ...
              <td class="right">
			  <?php foreach ($customer['actionprint'] as $actionprint) { ?>
                [ <a href="<?php echo $actionprint['href']; ?>"><?php echo $actionprint['text']; ?></a> ]
                <?php } ?>
...
function reset_filter() {
	url = 'index.php?route=service/satisfaction&token=<?php echo $token; ?>';
	location = url;
}
function filter() {
	url = 'index.php?route=service/satisfaction&token=<?php echo $token; ?>';
	
	var filter_date_written = $('input[name=\'filter_date_written\']').attr('value');
	
	if (filter_date_written) {
		url += '&filter_date_written=' + encodeURIComponent(filter_date_written);
	}
