<?php    
class ControllerNurseNurse2c extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('nurse/nurse2c');
	...
	public function word() {		
		...
		if ($customer_info) { 
			...
			$filename = "媽媽每日護理紀錄_".$customer_info['name'];
			
			$this->model_tool_worddoc->clsMsDocGenerator('PORTRAIT','A4','',1,1,1,1);
			$this->model_tool_worddoc->setDocumentCharset('UTF-8');
					
			$nurse2c_infos = $this->model_nurse_nurse2c->getNurse2cs($this->request->get['customer_id']);
			
				$rooms = $this->model_service_customer->getTransfers($this->request->get['customer_id']);
				 if (!empty($rooms)) {
					...
					$customer_firstroom = '未指定房間';
					$customer_transfers = array();
					$customer_lastroom = '未指定房間';
				 }
					...
					$tblstyle = array(
						...
						$this->model_tool_worddoc->addParagraph("媽媽每日護理紀錄", $titleFormat);
						$this->model_tool_worddoc->addParagraph("房號：".$customer_lastroom."<span style='mso-tab-count:1'></span>媽媽姓名：".$customer_info['name']."<span style='mso-tab-count:2'></span>生產日：".$customer_info['partum_date']."<span style='mso-tab-count:2'></span>生產方式：".$customer_info['childbirth_method']."<span style='mso-tab-count:1'></span>胎次：".$customer_info['child_count_p'], $center);
			
			foreach ($nurse2c_infos as $kk => $result) {
				...
				$table3[] = array (
						'content' => array('精神狀況：', $result['chk17'], '日期：'.$result['date_added'] . "<span style='mso-tab-count:2'></span>" . '產後天數：' . $result['days_afterbirth']), 
						'cellwidth'	=> array('20%', '30%', '50%'),
						'span' => array('0','0','2')
					);

				$table3[] = array (
						'content' => array('惡露量：', $result['chk9'], '體溫：'.$result['chk24'].'度'. "<span style='mso-tab-count:2'></span>" .'宮底高度：'.$result['chk8']), 
						'cellwidth'	=> array('20%', '30%', '50%'),
						'span' => array('0','0','2')
					);
					
				$table3[] = array (
						'content' => array('惡露顏色：', $result['chk10'], '排泄情形：', $result['chk22']), 
						'cellwidth'	=> array('20%', '30%', '20%', '30%'),
						'span' => array()
					);
					
				$table3[] = array (
						'content' => array('惡露性質：', $result['chk11'], '疼痛情形：', $result['chk23']), 
						'cellwidth'	=> array('20%', '30%', '20%', '30%'),
						'span' => array()
					);
					
				$chk5array = (is_serialized($result['chk5']))? unserialize($result['chk5']) : array();
				$chk5 = (!empty($chk5array)) ? implode(', ', $chk5array) : '';
				$table3[] = array (
						'content' => array('乳房狀況-左：', $chk5, '活動情形', $result['chk21']), 
						'cellwidth'	=> array('20%', '30%', '20%', '30%'),
						'span' => array()
					);
					
				$chk6array = (is_serialized($result['chk6']))? unserialize($result['chk6']) : array();
				$chk6 = (!empty($chk6array)) ? implode(', ', $chk6array) : '';
				$table3[] = array (
						'content' => array('乳房狀況-右：', $chk6, ''), 
						'cellwidth'	=> array('20%', '30%', '50%'),
						'rspan' => array('0','0','2'),
						'span' => array('0', '0', '2')
					);
					
				$chk1array = (is_serialized($result['chk1']))? unserialize($result['chk1']) : array();
				$chk1 = (!empty($chk1array)) ? implode(', ', $chk1array) : '';
				$table3[] = array (
						'content' => array('乳頭狀況-左：', $chk1), 
						'cellwidth'	=> array('20%', '30%'),
						'span' => array()
					);
					
				$chk2array = (is_serialized($result['chk2']))? unserialize($result['chk2']) : array();
				$chk2 = (!empty($chk2array)) ? implode(', ', $chk2array) : '';
				$table3[] = array (
						'content' => array('乳頭狀況-右：', $chk2, ''), 
						'cellwidth'	=> array('20%', '30%', '50%'),
						'rspan' => array('0','0','2'),
						'span' => array('0', '0', '2')
					);
				...
				$table3[] = array (
						'content' => array('會陰傷口：', $result['chk12'] ), 
						'cellwidth'	=> array('15%', '35%'),
						'span' => array('0', '0', '2')
					);		
					
				if ($customer_info['childbirth_method'] == 'C/S') {
					$table3[] = array (
							'content' => array('C/S傷口cover：', $result['chk13'], '' ), 
							'cellwidth'	=> array('20%', '30%', '50%'),
							'rspan' => array('0','0','2'),
							'span' => array('0', '0', '2')
						);	
					...
					$table3[] = array (
							'content' => array('C/S傷口外觀：', $result['chk14']), 
							'cellwidth'	=> array('20%', '30%'),
							'span' => array('0', '0')
						);	
				}
				...
				if ($table3) {
					...
					$this->model_tool_worddoc->startTable($tblstyle);
				...
				if (!(($kk+1)%3) && $kk+1 <> count($nurse2c_infos)) {
  	...
  	public function store() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_nurse_nurse2c->addNurse2c($this->request->post);
			...
			//$this->redirect($this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			$this->redirect($this->url->link('nurse/nurse2c/insert', 'token=' . $this->session->data['token'] . '&customer_id='. $this->request->get['customer_id'] . $url, 'SSL'));
		}
...
*/			
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_nurse_nurse2c->editNurse2c($this->request->get['customer_nurse2c_id'], $this->request->post);
  	...
  	public function delete() {
    	...
    	if (isset($this->request->get['id']) && $this->request->get['id']<>'' && $this->validateDelete()) {
			$this->model_nurse_nurse2c->deleteNurse2c($this->request->get['customer_id'],$this->request->get['id']);
			...
			$this->redirect($this->url->link('nurse/nurse2c/insert', 'token=' . $this->session->data['token'] . $url, 'SSL'));
    	}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    	...
    	foreach ($results as $result) {
			$form2cs = $this->model_nurse_nurse2c->getNurse2cs($result['customer_id']);
			
			$action = array();
		
			/*
			if (!empty($form2cs)) {
				foreach ($form2cs as $f2c) {
					$action[] = array(
						'text' => $this->language->get('text_edit').$f2c['date_added'],
						'href' => $this->url->link('nurse/nurse2c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_nurse2c_id=' . $f2c['customer_nurse2c_id'] . $url, 'SSL')
					);
				}
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
					'href' => $this->url->link('nurse/nurse2c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);		
			} else {
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
			...
			*/
			...
			if (!empty($form2cs)) {
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('nurse/nurse2c/word', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);	
				/*
				$action[] = array(
					'text' => '觀看統計',
		...
		$this->data['sort_name'] = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_room_id'] = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . '&sort=customer_room_id' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
		$this->data['sort_service'] = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . '&sort=service' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'nurse/nurse2c_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['button_add_nurse2c'] = $this->language->get('button_add_nurse2c');
		...
		$this->data['tab_nurse2c'] = $this->language->get('tab_nurse2c');
		$this->data['tab_nurse2cform'] = $this->language->get('tab_nurse2cform');
		...
		if (!isset($this->request->get['customer_id'])) { $this->redirect($this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . $url, 'SSL')); }
		
		/*
		if (!isset($this->request->get['customer_nurse2c_id'])) {
			$this->data['action'] = $this->url->link('nurse/nurse2c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('nurse/nurse2c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&customer_nurse2c_id=' . $this->request->get['customer_nurse2c_id'] . $url, 'SSL');
		}
		*/
		
		$this->data['action'] = $this->url->link('nurse/nurse2c/store', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');

    	$this->data['cancel'] = $this->url->link('nurse/nurse2c', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$customer_nurse2c_id = (isset($this->request->get['customer_nurse2c_id'])) ?  $this->request->get['customer_nurse2c_id'] : '';
      		
			$form_info = $this->model_nurse_nurse2c->getNurse2c($this->request->get['customer_id'], $customer_nurse2c_id);
			$datatables = $this->model_nurse_nurse2c->getDataTables($this->request->get['customer_id']);
    	}

		$this->load->model('service/customer');
		$this->data['partumdate'] = $this->model_service_customer->getCustomerPartumDate($this->request->get['customer_id']);
		
#!#####

		if (isset($this->request->post['nurse2cform'])) { 
      		$this->data['nurse2cforms'] = $this->request->post['nurse2cform'];
		} elseif (!empty($this->request->get['customer_id'])) {
			$this->data['nurse2cforms'] = $this->model_nurse_nurse2c->getNurse2cs($this->request->get['customer_id']);
		} else {
			$this->data['nurse2cforms'] = array();
    	}	
...
*/
		...
		*/
		...
		$this->template = 'nurse/nurse2c_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'nurse/nurse2c')) {
  	...
  	private function validateDelete() {
    	if (!$this->user->hasPermission('delete', 'nurse/nurse2c')) {
      		$this->error['warning'] = $this->language->get('error_permission');
    	}	
