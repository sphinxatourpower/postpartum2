...
class ControllerNurseNurse3b extends Controller { 
	...
	public function word() {		
		...
		$this->model_tool_worddoc->addParagraph("寶寶護理記錄<span style='mso-tab-count:1'></span>".$childname, $titleFormat);
		$this->model_tool_worddoc->addParagraph("房號：".$customer_lastroom."<span style='mso-tab-count:1'></span>媽媽姓名：".$datatable['customer_name']."<span style='mso-tab-count:1'></span>出生體重：".$datatable['birthweight']."g<span style='mso-tab-count:1'></span>生產方式：".$datatable['childbirth_method']."<span style='mso-tab-count:1'></span>性別：".$datatable['gender'], $center);
		...
		if ($results) { 
			...
			foreach ($results as $kk => $result) {			
				...
				$table3[] = array (
						'content' => array('精神狀況：', $result['chk24'], '日期：', $result['date_added'] . "&#32;" . $result['arrangement']), 
						'cellwidth'	=> array('15%', '35%', '15%', '35%'),
						'span' => array()
					);

				$table3[] = array (
						'content' => array('吸吮力：', $result['chk25'], '體溫：'.$result['chk1'].'度'. "<span style='mso-tab-count:2'></span>" .'體重：'.$result['chk10'].'g'), 
						'cellwidth'	=> array('15%', '35%', '50%'),
						'span' => array('0','0','2')
					);
					
				$chk23array = (is_serialized($result['chk23supplement']))? unserialize($result['chk23supplement']) : array();
				$chk23 = '';
				foreach ($chk23array as $v) {
					$chk23 .= 'ｖ'.$v."&#32;";
				}
				$table3[] = array (
						'content' => array('活動力：', $result['chk26'], $chk23), 
						'cellwidth'	=> array('15%', '35%', '50%'),
						'span' => array('0','0','2')
					);
					
				$chk19array = (is_serialized($result['chk19supplement']))? unserialize($result['chk19supplement']) : array();
				$chk19 = (!empty($chk19array)) ? '：' . implode(', ', $chk19array) : '';
				$table3[] = array (
						'content' => array('紅臀：', $result['chk22'], '卡介苗', $result['chk19'] . $chk19 ), 
						'cellwidth'	=> array('15%', '35%', '15%', '35%'),
						'span' => array()
					);
			
				$table3[] = array (
						'content' => array('膚色：', $result['chk20'], '黃疸值：', $result['chk13'] . 'mg/dl' ), 
						'cellwidth'	=> array('15%', '35%', '15%', '35%'),
						'span' => array()
					);		
					
				$chk20array = (is_serialized($result['chk20supplement']))? unserialize($result['chk20supplement']) : array();
				$chk20 = (!empty($chk20array)) ? implode(', ', $chk20array) : '';
				$table3[] = array (
						'content' => array('皮膚狀況：', $chk20, '補充說明：' . $result['supplement'] ), 
						'cellwidth'	=> array('15%', '35%', '50%'),
						'rspan' => array('0','0','2'),
						'span' => array('0', '0', '2')
					);	
				...
				$table3[] = array (
						'content' => array('小便狀況：', $result['chk21'] . "<span style='mso-tab-count:2'></span>" . '累積次數：' . $result['chk11'] . '次' ), 
						'cellwidth'	=> array('15%', '35%'),
						'span' => array('0', '0')
					);		
					
				$table3[] = array (
						'content' => array('大便顏色：', $result['chk15'] . "<span style='mso-tab-count:2'></span>" . '累積次數：' . $result['chk12'] . '次', '' ), 
						'cellwidth'	=> array('15%', '35%', '50%'),
						'rspan' => array('0','0','2'),
						'span' => array('0', '0', '2')
					);	
				...
				$table3[] = array (
						'content' => array('大便性質：', $result['chk16'] ), 
						'cellwidth'	=> array('15%', '35%'),
						'span' => array('0', '0')
					);	
					
				$table3[] = array (
						'content' => array('肚臍狀況：', $result['chk17'], '' ), 
						'cellwidth'	=> array('15%', '35%', '50%'),
						'rspan' => array('0','0','2'),
						'span' => array('0', '0', '2')
					);	
					
				$chk18array = (is_serialized($result['chk18']))? unserialize($result['chk18']) : array();
				$chk18 = (!empty($chk18array)) ? implode(', ', $chk18array) : '';
				$table3[] = array (
						'content' => array('肚臍外觀：', $chk18), 
						'cellwidth'	=> array('15%', '35%'),
						'span' => array('0', '0')
					);	
...
#!#####
		...
		if (isset($this->request->post['chk19supplement'])) {
      		$this->data['chk19supplement'] = $this->request->post['chk19supplement'];
    	} elseif (!empty($form_info)) { 
			$this->data['chk19supplement'] = $form_info['chk19supplement'];
		} else {
      		$this->data['chk19supplement'] = '';
    	}	
		
		if (isset($this->request->post['chk20supplement'])) {
      		$this->data['chk20supplement'] = $this->request->post['chk20supplement'];
    	} elseif (!empty($form_info)) { 
			$this->data['chk20supplement'] = $form_info['chk20supplement'];
		} else {
      		$this->data['chk20supplement'] = '';
    	}	
		
		if (isset($this->request->post['chk23supplement'])) {
      		$this->data['chk23supplement'] = $this->request->post['chk23supplement'];
    	} elseif (!empty($form_info)) { 
			$this->data['chk23supplement'] = $form_info['chk23supplement'];
		} else {
      		$this->data['chk23supplement'] = '';
    	}	
		*/
