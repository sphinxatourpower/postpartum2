<?php    
class ControllerNurseNurse4c extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('nurse/nurse4c');
	...
	public function word() {		
		...
		if ($customer_info) { 
			...
			$nurse4c_info = $this->model_nurse_nurse4c->getNurse4c($this->request->get['customer_id']);
			$results = $this->model_nurse_nurse4c->getDataTables($this->request->get['customer_id']);
			
			$filename = "4C愛丁堡產後憂鬱量表_".$customer_info['name'];
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_nurse_nurse4c->addNurse4c($this->request->post);
...
*/			
			...
			$this->redirect($this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_nurse_nurse4c->editNurse4c($this->request->get['customer_nurse4c_id'], $this->request->post);
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    	...
    	foreach ($results as $result) {
			$form4c = $this->model_nurse_nurse4c->getNurse4c($result['customer_id']);
			...
			if (!empty($form4c)) {
				$action[] = array(
					...
					'href' => $this->url->link('nurse/nurse4c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_nurse4c_id=' . $form4c['customer_nurse4c_id'] . $url, 'SSL')
				);
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('nurse/nurse4c/word', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_nurse4c_id=' . $form4c['customer_nurse4c_id'] . $url, 'SSL')
				);
			} else {
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
					'href' => $this->url->link('nurse/nurse4c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);				
			}
		...
		$this->data['sort_name'] = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_room_id'] = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . '&sort=customer_room_id' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
		$this->data['sort_service'] = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . '&sort=service' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'nurse/nurse4c_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['tab_nurse4c'] = $this->language->get('tab_nurse4c');
		...
		*/
		...
		if (!isset($this->request->get['customer_nurse4c_id'])) {
			$this->data['action'] = $this->url->link('nurse/nurse4c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('nurse/nurse4c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&customer_nurse4c_id=' . $this->request->get['customer_nurse4c_id'] . $url, 'SSL');
		}


    	$this->data['cancel'] = $this->url->link('nurse/nurse4c', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$form_info = $this->model_nurse_nurse4c->getNurse4c($this->request->get['customer_id']);
			$datatables = $this->model_nurse_nurse4c->getDataTables($this->request->get['customer_id']);
    	}
...
*/
		...
		*/
		...
		$this->template = 'nurse/nurse4c_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'nurse/nurse4c')) {
