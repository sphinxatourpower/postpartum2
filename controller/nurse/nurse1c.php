<?php    
class ControllerNurseNurse1c extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('nurse/nurse1c');
	...
	public function word() {		
		...
		if ($customer_info) { 
			...
			$nurse1c_info = $this->model_nurse_nurse1c->getNurse1c($this->request->get['customer_id']);
			$results = $this->model_nurse_nurse1c->getDataTables($this->request->get['customer_id']);
			
			$filename = "媽媽入住評估表_".$customer_info['name'];
			...
			foreach ($results as $result) {		
				...
				$this->model_tool_worddoc->addParagraph(PCOMPANY, $centertitleFormat);
				$this->model_tool_worddoc->addParagraph("媽媽入住評估與指導表", $centertitleFormat);
				//$this->model_tool_worddoc->addParagraph("表格 1");
				
				$this->model_tool_worddoc->addParagraph("★入住評估：" . $customer_info['name'], $titleFormat);
				...
				*/
				...
				if ($nurse1c_info) {					
					$this->model_tool_worddoc->addParagraph("1. 目前身高：".$nurse1c_info['chk10']." cm" . "<span style='mso-tab-count:2'></span>體重：" .$nurse1c_info['chk11']. " kg");
					$this->model_tool_worddoc->addParagraph("2. V/S：T：".$nurse1c_info['chk1'] . "<span style='mso-tab-count:2'></span>P：" . $nurse1c_info['chk2']. "<span style='mso-tab-count:2'></span>R：".$nurse1c_info['chk3']. "<span style='mso-tab-count:2'></span>BP：".$nurse1c_info['chk4a']."/".$nurse1c_info['chk4b']."<span style='mso-tab-count:2'></span>生產週數：" . $customer_info['pregnancy_weeks'] . "週 " . (int)$customer_info['weeks_plus'] . "日");
					$this->model_tool_worddoc->addParagraph("3. 生產方式：" . $customer_info['childbirth_method'] );
					
					$pattern = '/其他:/i';
					$replacement = '';
					$childbirth_hospital = preg_replace($pattern, $replacement, $customer_info['childbirth_hospital']);
					$this->model_tool_worddoc->addParagraph("4. 生產日期：".$customer_info['partum_date']."<span style='mso-tab-count:2'></span>生產醫院：" . $childbirth_hospital  );
					
					$chk6array = (is_serialized($nurse1c_info['chk6']))? unserialize($nurse1c_info['chk6']) : array();
					$chk6 = (!empty($chk6array)) ? implode(', ', $chk6array) : '無';
					$this->model_tool_worddoc->addParagraph("5. 生產期合併症：".$chk6 );
					...
					*/
					...
					$this->model_tool_worddoc->addParagraph('★入住指導項目:', $titleFormat);
					$this->model_tool_worddoc->addParagraph('□機構簽訂保障雙方權益及義務之契約');
					$this->model_tool_worddoc->addParagraph('□符合當地政府公告之收費標準');
					$this->model_tool_worddoc->addParagraph('□機構無推銷或出售瘦身、美容、奶粉、藥品等相關產品');
					$this->model_tool_worddoc->addParagraph('□顧客意見反映管道');
					$this->model_tool_worddoc->addParagraph('□訪客陪客作業管理規範');
					$this->model_tool_worddoc->addParagraph('□嬰兒安全措施(門禁房卡、監視器、手/腳圈辨識、床頭卡)');
					$this->model_tool_worddoc->addParagraph('□感染控制預防措施');
					$this->model_tool_worddoc->addParagraph('□滅火器、消防栓、逃生袋擺設位置及逃生路線');
					$this->model_tool_worddoc->addParagraph('□緊急呼叫設備的位置及正確操作方法');
					$this->model_tool_worddoc->addParagraph('□有實施滿意度調查');
					$this->model_tool_worddoc->addParagraph('□提供相關照護及指導服務');
					$this->model_tool_worddoc->addParagraph('□支持母乳哺育政策');
					$this->model_tool_worddoc->addParagraph('□親子同室的好處並訂有相關機制');
					$this->model_tool_worddoc->addParagraph('□親子同室感染控制及安全措施');

					$this->model_tool_worddoc->addParagraph('');
					
					$this->model_tool_worddoc->addParagraph('日期：' . $nurse1c_info['date_added'] . "<span style='mso-tab-count:2'></span>" . '指導人：'.$nurse_name , $right);
				}
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_nurse_nurse1c->addNurse1c($this->request->post);
...
*/			
			...
			$this->redirect($this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_nurse_nurse1c->editNurse1c($this->request->get['customer_nurse1c_id'], $this->request->post);
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    	...
    	foreach ($results as $result) {
			$form1c = $this->model_nurse_nurse1c->getNurse1c($result['customer_id']);
			...
			if (!empty($form1c)) {
				$action[] = array(
					...
					'href' => $this->url->link('nurse/nurse1c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_nurse1c_id=' . $form1c['customer_nurse1c_id'] . $url, 'SSL')
				);
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('nurse/nurse1c/word', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_nurse1c_id=' . $form1c['customer_nurse1c_id'] . $url, 'SSL')
				);
			} else {
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
					'href' => $this->url->link('nurse/nurse1c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);				
			}
		...
		$this->data['sort_name'] = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_room_id'] = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . '&sort=customer_room_id' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
		$this->data['sort_service'] = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . '&sort=service' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'nurse/nurse1c_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['tab_nurse1c'] = $this->language->get('tab_nurse1c');
		...
		*/
		...
		if (!isset($this->request->get['customer_nurse1c_id'])) {
			$this->data['action'] = $this->url->link('nurse/nurse1c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('nurse/nurse1c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&customer_nurse1c_id=' . $this->request->get['customer_nurse1c_id'] . $url, 'SSL');
		}


    	$this->data['cancel'] = $this->url->link('nurse/nurse1c', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$form_info = $this->model_nurse_nurse1c->getNurse1c($this->request->get['customer_id']);
			$datatables = $this->model_nurse_nurse1c->getDataTables($this->request->get['customer_id']);
    	}
...
*/
		...
		*/
		...
		$this->template = 'nurse/nurse1c_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'nurse/nurse1c')) {
