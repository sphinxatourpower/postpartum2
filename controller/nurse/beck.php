<?php    
class ControllerNurseBeck extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('nurse/beck');
	...
	public function word() {		
		...
		if ($customer_info) { 
			...
			$beck_info = $this->model_nurse_beck->getNurse4c($this->request->get['customer_id']);
			$results = $this->model_nurse_beck->getDataTables($this->request->get['customer_id']);
			
			$filename = "4c貝氏產後憂鬱量表_".$customer_info['name'];
			...
			foreach ($results as $result) {		
				...
				$this->model_tool_worddoc->addParagraph("4C貝氏堡產後憂鬱量表：".$beck_info['date_added'], $titleFormat);
				...
				if ($beck_info) {
					...
					$this->model_tool_worddoc->addParagraph("得分：".$beck_info['score']);
					$this->model_tool_worddoc->addParagraph('在過去七天中');
					
					if ($beck_info['chk1']<>'') {
						$this->model_tool_worddoc->addParagraph("A.<br />ｖ".$beck_info['chk1']);
					}
					if ($beck_info['chk2']<>'') {
						$this->model_tool_worddoc->addParagraph("B.<br />ｖ".$beck_info['chk2']);
					}
					if ($beck_info['chk3']<>'') {
						$this->model_tool_worddoc->addParagraph("C.<br />ｖ".$beck_info['chk3']);
					}
					if ($beck_info['chk4']<>'') {
						$this->model_tool_worddoc->addParagraph("D.<br />ｖ".$beck_info['chk4']);
					}
					if ($beck_info['chk5']<>'') {
						$this->model_tool_worddoc->addParagraph("E.<br />ｖ".$beck_info['chk5']);
					}
					if ($beck_info['chk6']<>'') {
						$this->model_tool_worddoc->addParagraph("F.<br />ｖ".$beck_info['chk6']);
					}
					if ($beck_info['chk7']<>'') {
						$this->model_tool_worddoc->addParagraph("G.<br />ｖ".$beck_info['chk7']);
					}
					if ($beck_info['chk8']<>'') {
						$this->model_tool_worddoc->addParagraph("H.<br />ｖ".$beck_info['chk8']);
					}
					if ($beck_info['chk9']<>'') {
						$this->model_tool_worddoc->addParagraph("I.<br />ｖ".$beck_info['chk9']);
					}
					if ($beck_info['chk10']<>'') {
						$this->model_tool_worddoc->addParagraph("J.<br />ｖ".$beck_info['chk10']);
					}
					if ($beck_info['chk11']<>'') {
						$this->model_tool_worddoc->addParagraph("K.<br />ｖ".$beck_info['chk11']);
					}
					if ($beck_info['chk12']<>'') {
						$this->model_tool_worddoc->addParagraph("L.<br />ｖ".$beck_info['chk12']);
					}
					if ($beck_info['chk13']<>'') {
						$this->model_tool_worddoc->addParagraph("M.<br />ｖ".$beck_info['chk13']);
					}
					if ($beck_info['chk14']<>'') {
						$this->model_tool_worddoc->addParagraph("N.<br />ｖ".$beck_info['chk14']);
					}
					if ($beck_info['chk15']<>'') {
						$this->model_tool_worddoc->addParagraph("O.<br />ｖ".$beck_info['chk15']);
					}
					if ($beck_info['chk16']<>'') {
						$this->model_tool_worddoc->addParagraph("P.<br />ｖ".$beck_info['chk16']);
					}
					if ($beck_info['chk17']<>'') {
						$this->model_tool_worddoc->addParagraph("Q.<br />ｖ".$beck_info['chk17']);
					}
					if ($beck_info['chk18']<>'') {
						$this->model_tool_worddoc->addParagraph("R.<br />ｖ".$beck_info['chk18']);
					}
					if ($beck_info['chk19']<>'') {
						$this->model_tool_worddoc->addParagraph("S.<br />ｖ".$beck_info['chk19']);
					}
					if ($beck_info['chk20']<>'') {
						$this->model_tool_worddoc->addParagraph("T.<br />ｖ".$beck_info['chk20']);
					}
					if ($beck_info['chk21']<>'') {
						$this->model_tool_worddoc->addParagraph("U.<br />ｖ".$beck_info['chk21']);
					}
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_nurse_beck->addNurse4c($this->request->post);
...
*/			
			...
			$this->redirect($this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_nurse_beck->editNurse4c($this->request->get['customer_beck_id'], $this->request->post);
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    	...
    	foreach ($results as $result) {
			$form4c = $this->model_nurse_beck->getNurse4c($result['customer_id']);
			...
			if (!empty($form4c)) {
				$action[] = array(
					...
					'href' => $this->url->link('nurse/beck/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_beck_id=' . $form4c['customer_beck_id'] . $url, 'SSL')
				);
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('nurse/beck/word', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_beck_id=' . $form4c['customer_beck_id'] . $url, 'SSL')
				);
			} else {
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
					'href' => $this->url->link('nurse/beck/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);				
			}
		...
		$this->data['sort_name'] = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_room_id'] = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . '&sort=customer_room_id' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
		$this->data['sort_service'] = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . '&sort=service' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'nurse/beck_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['tab_beck'] = $this->language->get('tab_beck');
		...
		*/
		...
		if (!isset($this->request->get['customer_beck_id'])) {
			$this->data['action'] = $this->url->link('nurse/beck/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('nurse/beck/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&customer_beck_id=' . $this->request->get['customer_beck_id'] . $url, 'SSL');
		}


    	$this->data['cancel'] = $this->url->link('nurse/beck', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$form_info = $this->model_nurse_beck->getNurse4c($this->request->get['customer_id']);
			$datatables = $this->model_nurse_beck->getDataTables($this->request->get['customer_id']);
    	}
...
*/
		...
		*/
		...
		$this->template = 'nurse/beck_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'nurse/beck')) {
