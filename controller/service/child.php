<?php    
class ControllerServiceChild extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('service/child');
	...
	public function word() {		
		...
		if ($result) { 
				...
				$service_name = $this->model_service_customer_service->getCustomerServiceName($result['service_id']);
				...
				$images = $this->model_service_child->getChildImages($result['customer_child_id']);
				
				$this->load->model('tool/image');
				//print $this->model_tool_image->resize($images[0]['image'], '', 300 );
				//exit;
				
				/*
				if (!empty($images) && $images['image'] && file_exists(DIR_IMAGE . $images['image'])) {
					print $this->model_tool_image->resize($images['image'], 300, 300);
				}
				*/
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_service_child->addChild($this->request->post);
...
*/			
			...
			$this->redirect($this->url->link('service/child', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_service_child->editChild($this->request->get['customer_child_id'], $this->request->post);
...
/*				
		if (isset($this->request->get['filter_customer_service_id'])) {
			$filter_customer_service_id = $this->request->get['filter_customer_service_id'];
		} else {
			$filter_customer_service_id = null;
		}
*/		
		if (isset($this->request->get['sort'])) {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('service/child', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => $this->language->get('text_separator')
   		);
		
		//$this->data['insert'] = $this->url->link('service/child/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		//$this->data['delete'] = $this->url->link('service/child/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
//			'filter_customer_service_id' => $filter_customer_service_id, 
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('service/child/update', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $result['customer_child_id'] . $url, 'SSL')
			);
			...
			$actionprint[] = array(
				//'text' => $this->language->get('text_edit'),
				'text' => '下載列印',
				'href' => $this->url->link('service/child/word', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $result['customer_child_id'] . $url, 'SSL')
			);
...
*/
		...
		$this->data['sort_name'] = $this->url->link('service/child', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_customer_name'] = $this->url->link('service/child', 'token=' . $this->session->data['token'] . '&sort=customer_name' . $url, 'SSL');
		$this->data['sort_birthdate'] = $this->url->link('service/child', 'token=' . $this->session->data['token'] . '&sort=c.birthdate' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('service/child', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
...
*/
		...
		$pagination->url = $this->url->link('service/child', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'service/child_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		*/
		...
		if (!isset($this->request->get['customer_child_id'])) {
			...
			$this->data['action'] = $this->url->link('service/child/update', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $this->request->get['customer_child_id'] . $url, 'SSL');
		}
		  
    	$this->data['cancel'] = $this->url->link('service/child', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->get['customer_child_id'])) {
			...
			$this->data['child_name'] = $this->model_service_child->getChildName($form_info['customer_child_id']);
		} else {
      		$this->data['customer_child_id'] = 0;
...
*/
		...
		*/
		...
		if (isset($this->request->post['image'])) {
			...
			$this->data['image'] = $child_info['image'];
		} else {
			$this->data['image'] = '';
		}
		...
		} elseif (!empty($child_info) && $child_info['image'] && file_exists(DIR_IMAGE . $child_info['image'])) {
			$this->data['thumb'] = $this->model_tool_image->resize($child_info['image'], 100, 100);
		} else {
			$this->data['thumb'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}
		
		if (isset($this->request->post['child_image'])) {
			$child_images = $this->request->post['child_image'];
		} elseif (isset($this->request->get['customer_child_id'])) {
			$child_images = $this->model_service_child->getChildImages($this->request->get['customer_child_id']);
		} else {
			$child_images = array();
		}
		
		$this->data['child_images'] = array();
		
		foreach ($child_images as $child_image) {
			if ($child_image['image'] && file_exists(DIR_IMAGE . $child_image['image'])) {
			...
			$this->data['child_images'][] = array(
				'image'      => $image,
				'image_txt'	=> $child_image['image_txt'],
				'thumb'      => $this->model_tool_image->resize($image, 100, 100),
				'sort_order' => $child_image['sort_order']
			);
		}

		$this->data['no_image'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		
		$this->template = 'service/child_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'service/child')) {
