<?php    
class ControllerBreastBreast1c extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('breast/breast1c');
		 
		$this->document->setTitle($this->language->get('heading_title'));
		
		$this->load->model('breast/breast1c');
	...
	public function word() {		
		...
		if ($customer_info) { 
			...
			$breast1c_infos = $this->model_breast_breast1c->getBreast1cs($this->request->get['customer_id']);
			
			foreach ($breast1c_infos as $kk => $breast1c_info) {
				
				$filename = "2C產後衛教每日身評紀錄單_".$customer_info['name'];
				
				$this->load->model('service/customer_service');
				$nurse_name = $this->model_service_customer_service->getCustomerServiceName($breast1c_info['nurse_id']);
					...
					if ($breast1c_info) {
						$this->model_tool_worddoc->addParagraph("2C產後衛教每日身評紀錄單：".$breast1c_info['date_added'], $titleFormat);
					
						$this->model_tool_worddoc->addParagraph("護理人員：".$nurse_name);
						$this->model_tool_worddoc->addParagraph("產後天數：".$breast1c_info['days_afterbirth']."，入住天數：".$breast1c_info['days_livein']);
						
						$this->model_tool_worddoc->addParagraph("");
						$this->model_tool_worddoc->addParagraph("《身體評估》", array("font-size"=>'14pt', 'font-weight'=>'bold'));
						
						if ($breast1c_info['chk24']<>'') {
							$this->model_tool_worddoc->addParagraph("體溫：".$breast1c_info['chk24']."℃");
						}
						if ($breast1c_info['chk25']<>'') {
							$this->model_tool_worddoc->addParagraph("脈搏：".$breast1c_info['chk25']."（每分鐘）");
						}
						if ($breast1c_info['chk26']<>'') {
							$this->model_tool_worddoc->addParagraph("呼吸：".$breast1c_info['chk26']."（每分鐘）");
						}
						if ($breast1c_info['chk27a']<>'' AND $breast1c_info['chk27b']<>'') {
							$this->model_tool_worddoc->addParagraph("血壓：(收縮壓):".$breast1c_info['chk27a']."mmHg&#32;(舒張壓):".$breast1c_info['chk27b']."mmHg");
						}
						if ($breast1c_info['chk1']<>'') {
							$this->model_tool_worddoc->addParagraph("右側乳頭外觀為：".$breast1c_info['chk1']);
						}
						if ($breast1c_info['chk2']<>'') {
							$this->model_tool_worddoc->addParagraph("左側乳頭外觀為：".$breast1c_info['chk2']);
						}
						if ($breast1c_info['chk3']<>'') {
							$this->model_tool_worddoc->addParagraph("右側乳暈外觀為：".$breast1c_info['chk3']);
						}
						if ($breast1c_info['chk4']<>'') {
							$this->model_tool_worddoc->addParagraph("左側乳暈外觀為：".$breast1c_info['chk4']);
						}
						if ($breast1c_info['chk5']<>'') {
							$this->model_tool_worddoc->addParagraph("右側乳房外觀為：".$breast1c_info['chk5']);
						}
						if ($breast1c_info['chk6']<>'') {
							$this->model_tool_worddoc->addParagraph("左側乳房外觀為：".$breast1c_info['chk6']);
						}
						if ($breast1c_info['chk7']<>'') {
							$this->model_tool_worddoc->addParagraph("單次雙側奶量為四小時：".$breast1c_info['chk7']);
						}
						if ($breast1c_info['chk8']<>'') {
							$this->model_tool_worddoc->addParagraph("宮底位置為：".$breast1c_info['chk8']);
						}
						if ($breast1c_info['chk9']<>'') {
							$this->model_tool_worddoc->addParagraph("惡露量為：".$breast1c_info['chk9']);
						}
						if ($breast1c_info['chk10']<>'') {
							$this->model_tool_worddoc->addParagraph("惡露顏色為：".$breast1c_info['chk10']);
						}
						if ($breast1c_info['chk11']<>'') {
							$this->model_tool_worddoc->addParagraph("惡露性質為：".$breast1c_info['chk11']);
						}
						if ($breast1c_info['chk12']<>'') {
							$this->model_tool_worddoc->addParagraph("該產婦 NSD 傷口為：".$breast1c_info['chk12']);
						}
						if ($breast1c_info['chk13']<>'') {
							$this->model_tool_worddoc->addParagraph("該產婦 C/S 傷口為：".$breast1c_info['chk13']);
						}
						if ($breast1c_info['chk14']<>'') {
							$this->model_tool_worddoc->addParagraph("痔瘡：".$breast1c_info['chk14']);
						}
						if ($breast1c_info['chk15']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦情緒為：".$breast1c_info['chk15']);
						}
						if ($breast1c_info['chk16']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦本日食慾：".$breast1c_info['chk16']);
						}
						if ($breast1c_info['chk17']<>'') {
							$this->model_tool_worddoc->addParagraph("活動精神狀況：".$breast1c_info['chk17']);
						}
						if ($breast1c_info['chk18']<>'') {
							$this->model_tool_worddoc->addParagraph("乳汁排洩情況：".$breast1c_info['chk18']);
						}
						if ($breast1c_info['chk19']<>'') {
							$this->model_tool_worddoc->addParagraph("乳房狀況：".$breast1c_info['chk19']);
						}
						if ($breast1c_info['chk20']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦按摩乳房狀況為：".$breast1c_info['chk20']);
						}
						if ($breast1c_info['chk21']<>'') {
							$this->model_tool_worddoc->addParagraph("學習餵奶打嗝技巧：".$breast1c_info['chk21']);
						}
						if ($breast1c_info['chk22']<>'') {
							$this->model_tool_worddoc->addParagraph("排尿情形：".$breast1c_info['chk22']);
						}
						if ($breast1c_info['chk23']<>'') {
							$this->model_tool_worddoc->addParagraph("排便狀況：".$breast1c_info['chk23']);
						}
						if (is_array($subedu = unserialize($breast1c_info['subject_healthedu'])) AND !empty($subedu)) {
							$textsubedu = '';
						...
						if ($breast1c_info['supplement']<>'') {
							$this->model_tool_worddoc->addParagraph("本日補充說明：<br />".nl2br($breast1c_info['supplement']));
						}
						
						$this->model_tool_worddoc->addParagraph("");
						$this->model_tool_worddoc->addParagraph("《資料紀錄》", array("font-size"=>'14pt', 'font-weight'=>'bold'));
						
						if ($breast1c_info['chk28']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦本日意識狀況為：".$breast1c_info['chk28']);
						}
						if ($breast1c_info['chk29']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦生產方式為：".$breast1c_info['chk29']."，傷口使用除疤凝膠");
						}
						if ($breast1c_info['chk30']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦身體不適，建議外出至：".$breast1c_info['chk30']."就醫");
						}
						if (is_array($c31 = unserialize($breast1c_info['chk31'])) AND !empty($c31)) {
							$textc31 = '';
							foreach ($c31 as $data) {
								$textc31 .= $data.",";
							}
						...
						if (is_array($c32 = unserialize($breast1c_info['chk32'])) AND !empty($c32)) {
							$textc32 = '';
							foreach ($c32 as $data) {
								$textc32 .= $data.",";
							}
						...
						if (is_array($c33 = unserialize($breast1c_info['chk33'])) AND !empty($c33)) {
							$textc33 = '';
							foreach ($c33 as $data) {
								$textc33 .= $data.",";
							}
						...
						if ($breast1c_info['chk34']<>'') {
							$this->model_tool_worddoc->addParagraph("已告知產婦，寶寶接觸到父母親以外的第三人，需在媽媽房隔離 ".$breast1c_info['chk34']);
						}
						if (is_array($c35 = unserialize($breast1c_info['chk35'])) AND !empty($c35)) {
							$textc35 = '';
							foreach ($c35 as $data) {
								$textc35 .= $data.",";
							}
						...
						if ($breast1c_info['chk36']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦有服藥，藥物為：".$breast1c_info['chk36']);
						}
						if ($breast1c_info['chk37a']<>'' AND $breast1c_info['chk37a']<>'0000-00-00 00:00:00') {
							$this->model_tool_worddoc->addParagraph("產婦本日外出，時間為：".$breast1c_info['chk37a']);
						}
						if ($breast1c_info['chk37b']<>'' AND $breast1c_info['chk37b']<>'0000-00-00 00:00:00') {
							$this->model_tool_worddoc->addParagraph("產婦返回，時間為：".$breast1c_info['chk37b']);
						}
						if (is_array($c38 = unserialize($breast1c_info['chk38'])) AND !empty($c38)) {
							$textc38 = '';
							foreach ($c38 as $data) {
								$textc38 .= $data.",";
							}
						...
						if (is_array($c39 = unserialize($breast1c_info['chk39'])) AND !empty($c39)) {
							$textc39 = '';
							foreach ($c39 as $data) {
								$textc39 .= $data.",";
							}
						...
						if (is_array($c40 = unserialize($breast1c_info['chk40'])) AND !empty($c40)) {
							$textc40 = '';
							foreach ($c40 as $data) {
								$textc40 .= $data.",";
							}
						...
						if (is_array($c41 = unserialize($breast1c_info['chk41'])) AND !empty($c41)) {
							$textc41 = '';
							foreach ($c41 as $data) {
								$textc41 .= $data.",";
							}
						...
						if ($breast1c_info['chk42']<>'') {
							$this->model_tool_worddoc->addParagraph("產婦基本狀況".$breast1c_info['chk42']."，身體健康，心情愉快，安心調養中");
						}
						if ($breast1c_info['supplement_record']<>'') {
							$this->model_tool_worddoc->addParagraph("特別補充事項：<br />".nl2br($breast1c_info['supplement_record']));
						}

					
					}

					if ($kk+1 < count($breast1c_infos)) {
  	...
  	public function store() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_breast_breast1c->addBreast1c($this->request->post);
			...
			//$this->redirect($this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			$this->redirect($this->url->link('breast/breast1c/insert', 'token=' . $this->session->data['token'] . '&customer_id='. $this->request->get['customer_id'] . $url, 'SSL'));
		}
...
*/			
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_breast_breast1c->editBreast1c($this->request->get['customer_breast1c_id'], $this->request->post);
  	...
  	public function delete() {
    	...
    	if (isset($this->request->get['id']) && $this->request->get['id']<>'' && $this->validateDelete()) {
			$this->model_breast_breast1c->deleteBreast1c($this->request->get['customer_id'],$this->request->get['id']);
			...
			$this->redirect($this->url->link('breast/breast1c/insert', 'token=' . $this->session->data['token'] . $url, 'SSL'));
    	}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    	...
    	foreach ($results as $result) {
			$form5cs = $this->model_breast_breast1c->getBreast1cs($result['customer_id']);
			...
			if (!empty($form5cs)) {
				foreach ($form5cs as $f5c) {
					$action[] = array(
						...
						'href' => $this->url->link('breast/breast1c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_breast1c_id=' . $f5c['customer_breast1c_id'] . $url, 'SSL')
					);
				}
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
					'href' => $this->url->link('breast/breast1c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);		
			} else {
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
			...
			*/
			...
			if (!empty($form5cs)) {
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('breast/breast1c/word', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);	
				$action[] = array(
					'text' => '觀看統計',
		...
		$this->data['sort_name'] = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_room_id'] = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . '&sort=customer_room_id' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
		$this->data['sort_service'] = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . '&sort=service' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'breast/breast1c_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['button_add_breast1c'] = $this->language->get('button_add_breast1c');
		...
		$this->data['tab_breast1c'] = $this->language->get('tab_breast1c');
		$this->data['tab_breast1cform'] = $this->language->get('tab_breast1cform');
		...
		if (!isset($this->request->get['customer_id'])) { $this->redirect($this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . $url, 'SSL')); }
		
		/*
		if (!isset($this->request->get['customer_breast1c_id'])) {
			$this->data['action'] = $this->url->link('breast/breast1c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('breast/breast1c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&customer_breast1c_id=' . $this->request->get['customer_breast1c_id'] . $url, 'SSL');
		}
		*/
		
		$this->data['action'] = $this->url->link('breast/breast1c/store', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');

    	$this->data['cancel'] = $this->url->link('breast/breast1c', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$customer_breast1c_id = (isset($this->request->get['customer_breast1c_id'])) ?  $this->request->get['customer_breast1c_id'] : '';
      		
			$form_info = $this->model_breast_breast1c->getBreast1c($this->request->get['customer_id'], $customer_breast1c_id);
			$datatables = $this->model_breast_breast1c->getDataTables($this->request->get['customer_id']);
    	}

#!#####
		
		if (isset($this->request->post['breast1cform'])) { 
      		$this->data['breast1cforms'] = $this->request->post['breast1cform'];
		} elseif (!empty($this->request->get['customer_id'])) {
			if ($this->user->getGroupId() == '1') {
				$this->data['breast1cforms'] = $this->model_breast_breast1c->getBreast1cs($this->request->get['customer_id']);
			} else {
				$this->data['breast1cforms'] = $this->model_breast_breast1c->getBreast1cs($this->request->get['customer_id'], $this->user->getId());
			}
		} else {
			$this->data['breast1cforms'] = array();
    	}	
...
*/
		...
		*/
		...
		$this->template = 'breast/breast1c_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'breast/breast1c')) {
  	...
  	private function validateDelete() {
    	if (!$this->user->hasPermission('delete', 'breast/breast1c')) {
      		$this->error['warning'] = $this->language->get('error_permission');
    	}	
