<?php    
class ControllerBreastBreast2c extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('breast/breast2c');
	...
	public function word() {		
		...
		if ($customer_info) { 
			...
			$breast2c_info = $this->model_breast_breast2c->getBreast2c($this->request->get['customer_id']);
			$results = $this->model_breast_breast2c->getDataTables($this->request->get['customer_id']);
			
			$filename = "1C產後衛教入住身體評估單_".$customer_info['name'];
			
			$this->load->model('service/customer_service');
			$nurse_name = $this->model_service_customer_service->getCustomerServiceName($breast2c_info['nurse_id']);
			...
			foreach ($results as $result) {		
				...
				$this->model_tool_worddoc->addParagraph("1C產後衛教入住身體評估單：".$breast2c_info['date_added'], $titleFormat);
				...
				if ($breast2c_info) {
					...
					if ($breast2c_info['chk1']<>'') {
						$this->model_tool_worddoc->addParagraph("體溫：".$breast2c_info['chk1']." ℃");
					}
					if ($breast2c_info['chk2']<>'') {
						$this->model_tool_worddoc->addParagraph("脈搏：".$breast2c_info['chk2']."（每分鐘）");
					}
					if ($breast2c_info['chk3']<>'') {
						$this->model_tool_worddoc->addParagraph("呼吸：".$breast2c_info['chk3']."（每分鐘）");
					}
					if ($breast2c_info['chk4a']<>'' AND $breast2c_info['chk4b']<>'') {
						$this->model_tool_worddoc->addParagraph("血壓：(收縮壓):".$breast2c_info['chk4a']."mmHg&#32;(舒張壓):".$breast2c_info['chk4b']."mmHg");
					}
					if ($breast2c_info['chk5']<>'') {
						$this->model_tool_worddoc->addParagraph("頸部：".$breast2c_info['chk5']);
					}
					if (is_array($c6 = unserialize($breast2c_info['chk6'])) AND !empty($c6)) {
						$text6 = '';
					...
					if (is_array($c7 = unserialize($breast2c_info['chk7'])) AND !empty($c7)) {
						$text7 = '';
					...
					if (is_array($c8 = unserialize($breast2c_info['chk8'])) AND !empty($c8)) {
						$text8 = '';
					...
					if (is_array($c9 = unserialize($breast2c_info['chk9'])) AND !empty($c9)) {
						$text9 = '';
					...
					if ($breast2c_info['chk10']<>'') {
						$this->model_tool_worddoc->addParagraph("乳頭(R)：".$breast2c_info['chk10']);
					}
					if ($breast2c_info['chk11']<>'') {
						$this->model_tool_worddoc->addParagraph("乳頭(L)：".$breast2c_info['chk11']);
					}
					if ($breast2c_info['chk12']<>'') {
						$this->model_tool_worddoc->addParagraph("乳暈(R)：".$breast2c_info['chk12']);
					}
					if ($breast2c_info['chk13']<>'') {
						$this->model_tool_worddoc->addParagraph("乳暈(L)：".$breast2c_info['chk13']);
					}
					if ($breast2c_info['chk14']<>'') {
						$this->model_tool_worddoc->addParagraph("乳房(R)：".$breast2c_info['chk14']);
					}
					if ($breast2c_info['chk15']<>'') {
						$this->model_tool_worddoc->addParagraph("乳房(L)：".$breast2c_info['chk15']);
					}
					if ($breast2c_info['chk16']<>'') {
						$this->model_tool_worddoc->addParagraph("子宮位置：".$breast2c_info['chk16']);
					}
					if ($breast2c_info['chk17']<>'') {
						$this->model_tool_worddoc->addParagraph("子宮：".$breast2c_info['chk17']);
					}
					if ($breast2c_info['chk18']<>'') {
						$this->model_tool_worddoc->addParagraph("惡露量：".$breast2c_info['chk18']);
					}
					if ($breast2c_info['chk19']<>'') {
						$this->model_tool_worddoc->addParagraph("惡露顏色：".$breast2c_info['chk19']);
					}
					if ($breast2c_info['chk20']<>'') {
						$this->model_tool_worddoc->addParagraph("惡露性質：".$breast2c_info['chk20']);
					}
					if (is_array($c21 = unserialize($breast2c_info['chk21'])) AND !empty($c21)) {
						$text21 = '';
					...
					if (is_array($c22 = unserialize($breast2c_info['chk22'])) AND !empty($c22)) {
						$text22 = '';
					...
					if (is_array($c23 = unserialize($breast2c_info['chk23'])) AND !empty($c23)) {
						$text23 = '';
					...
					if (is_array($c24 = unserialize($breast2c_info['chk24'])) AND !empty($c24)) {
						$text24 = '';
					...
					if (is_array($c25 = unserialize($breast2c_info['chk25'])) AND !empty($c25)) {
						$text25 = '';
					...
					if ($breast2c_info['chk26']<>'') {
						$this->model_tool_worddoc->addParagraph("痔瘡：".$breast2c_info['chk26']);
					}
					if ($breast2c_info['chk27']<>'') {
						$this->model_tool_worddoc->addParagraph("開刀史：".$breast2c_info['chk27']);
					}
					if ($breast2c_info['chk28']<>'') {
						$this->model_tool_worddoc->addParagraph("重大疾病：".$breast2c_info['chk28']);
					}
					if ($breast2c_info['chk29']<>'') {
						$this->model_tool_worddoc->addParagraph("服生化湯：".$breast2c_info['chk29']);
					}
					if ($breast2c_info['chk30']<>'') {
						$this->model_tool_worddoc->addParagraph("定期服用藥物：".$breast2c_info['chk30']);
					}
					if ($breast2c_info['chk31']<>'') {
						$this->model_tool_worddoc->addParagraph("服用子宮收縮劑：".$breast2c_info['chk31']);
					}
					if ($breast2c_info['chk32']<>'') {
						$this->model_tool_worddoc->addParagraph("家人一星期內有人感冒：".$breast2c_info['chk32']);
					}
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_breast_breast2c->addBreast2c($this->request->post);
...
*/			
			...
			$this->redirect($this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_breast_breast2c->editBreast2c($this->request->get['customer_breast2c_id'], $this->request->post);
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    	...
    	foreach ($results as $result) {
			$form1c = $this->model_breast_breast2c->getBreast2c($result['customer_id']);
			...
			if (!empty($form1c)) {
				$action[] = array(
					...
					'href' => $this->url->link('breast/breast2c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_breast2c_id=' . $form1c['customer_breast2c_id'] . $url, 'SSL')
				);
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('breast/breast2c/word', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . '&customer_breast2c_id=' . $form1c['customer_breast2c_id'] . $url, 'SSL')
				);
			} else {
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
					'href' => $this->url->link('breast/breast2c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);				
			}
		...
		$this->data['sort_name'] = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_room_id'] = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . '&sort=customer_room_id' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
		$this->data['sort_service'] = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . '&sort=service' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'breast/breast2c_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['tab_breast2c'] = $this->language->get('tab_breast2c');
		...
		*/
		...
		if (!isset($this->request->get['customer_breast2c_id'])) {
			$this->data['action'] = $this->url->link('breast/breast2c/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('breast/breast2c/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&customer_breast2c_id=' . $this->request->get['customer_breast2c_id'] . $url, 'SSL');
		}


    	$this->data['cancel'] = $this->url->link('breast/breast2c', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$form_info = $this->model_breast_breast2c->getBreast2c($this->request->get['customer_id']);
			$datatables = $this->model_breast_breast2c->getDataTables($this->request->get['customer_id']);
    	}
...
#!#####
		...
		if (isset($this->request->get['customer_id'])) {
			...
			$customer_info = $this->model_service_customer->getCustomer($form_info['customer_id']);
		...
		//print_r($this->data['customer_info']);
		$this->load->model('service/customer_service');
		...
		for ($i=1;$i<=34;$i++) {
				if (!empty($this->request->post)) {
					if ($i<6) {
						$this->data['chk'.$i.'a'] = $this->request->post['chk'.$i.'a'];
						$this->data['chk'.$i.'b'] = $this->request->post['chk'.$i.'b'];
					} else {
						$this->data['chk'.$i.''] = $this->request->post['chk'.$i.''];
						$this->data['chk'.$i.'_text'] = (isset($this->request->post['chk'.$i.'_text'])) ? $this->request->post['chk'.$i.'_text'] : '';
					}
				} elseif (!empty($form_info)) { 
					if ($i<6) {
						$this->data['chk'.$i.'a'] = $form_info['chk'.$i.'a'];				
						$this->data['chk'.$i.'b'] = $form_info['chk'.$i.'b'];								
					} else {
						$this->data['chk'.$i.''] = $form_info['chk'.$i.''];
						$this->data['chk'.$i.'_text'] = (isset($form_info['chk'.$i.'_text'])) ? $form_info['chk'.$i.'_text'] : '';
					}
				} else {
					if ($i<6) {
						$this->data['chk'.$i.'a'] = '';
...
/*
		...
		*/
		...
		if (isset($this->request->post['partum_date'])) {
      		$this->data['partum_date'] = $this->request->post['partum_date'];
    	} elseif (!empty($customer_info)) { 
			$this->data['partum_date'] = $customer_info['partum_date'];
		} else {
      		$this->data['partum_date'] = '';
    	}
...
*/
		...
		*/
		...
		$this->template = 'breast/breast2c_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'breast/breast2c')) {
