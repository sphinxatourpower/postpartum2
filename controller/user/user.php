...
class ControllerUserUser extends Controller {  
	private $error = array();
  	...
  	private function getList() {
		...
		foreach ($results as $result) {
			...
			$this->load->model('user/user_group');
      		...
      		$this->data['users'][] = array(
				...
				'date_end' => ($result['date_end']<>'' AND $result['date_end']<>'0000-00-00') ? date($this->language->get('date_format_short'), strtotime($result['date_end'])) : '無期限',
				'login' => ($result['date_login']<>'' AND $result['date_login']<>'0000-00-00 00:00:00') ? "登入時間：".date($this->language->get('date_format_short'), strtotime($result['date_login']))."<br />登入位址：".$result['ip'] : '未登入',
		...
		$this->data['column_login_record'] = $this->language->get('column_login_record');
		...
		$this->data['sort_user_group_id'] = $this->url->link('user/user', 'token=' . $this->session->data['token'] . '&sort=user_group_id' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('user/user', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
		$this->data['sort_date_end'] = $this->url->link('user/user', 'token=' . $this->session->data['token'] . '&sort=date_end' . $url, 'SSL');
