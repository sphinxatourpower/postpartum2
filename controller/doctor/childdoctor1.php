<?php    
class ControllerDoctorChildDoctor1 extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('doctor/childdoctor1');
	...
	public function word() {		
		...
		$result = $this->model_service_child->getChildDataTable($this->request->get['customer_child_id']);
		
		$this->load->model('doctor/childdoctor1');
		$childdoctor_info = $this->model_doctor_childdoctor1->getChildDoctor($this->request->get['customer_child_id']);
		...
		if ($result) { 
				...
				$filename = "1E嬰兒入住醫師評估單_".$childname;
				
				$rooms = $this->model_service_child->getTransfers($result['customer_id']);
				 if (!empty($rooms)) {
...
*/			
			...
			$this->redirect($this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => $this->language->get('text_separator')
   		);
		
		//$this->data['insert'] = $this->url->link('doctor/childdoctor1/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		//$this->data['delete'] = $this->url->link('doctor/childdoctor1/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
//			'filter_customer_service_id' => $filter_customer_service_id, 
    	...
    	foreach ($results as $result) {
			$drform = $this->model_doctor_childdoctor1->getChildDoctor($result['customer_child_id']);
			...
			if (!empty($drform)) {
				$action[] = array(
					...
					'href' => $this->url->link('doctor/childdoctor1/update', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $result['customer_child_id'] . '&customer_child_doctor_id=' . $drform['customer_child_doctor_id'] . $url, 'SSL')
				);
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('doctor/childdoctor1/word', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $result['customer_child_id'] . '&customer_child_doctor_id=' . $drform['customer_child_doctor_id'] . $url, 'SSL')
				);
			} else {
				$action[] = array(
					'text' => $this->language->get('text_addnew'),
					'href' => $this->url->link('doctor/childdoctor1/insert', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $result['customer_child_id'] .  $url, 'SSL')
				);				
			}
...
*/
		...
		$this->data['sort_name'] = $this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . '&sort=c.name' . $url, 'SSL');
		$this->data['sort_customer_name'] = $this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . '&sort=customer_name' . $url, 'SSL');
		$this->data['sort_birthdate'] = $this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . '&sort=c.birthdate' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
...
*/
		...
		$pagination->url = $this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'doctor/childdoctor1_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['tab_childdoctor1'] = $this->language->get('tab_childdoctor1');
		...
		if (isset($this->request->get['customer_child_doctor_id'])) {
			$this->data['customer_child_doctor_id'] = $this->request->get['customer_child_doctor_id'];
		} else {
			$this->data['customer_child_doctor_id'] = 0;
		}
		...
		*/
		...
		if (!isset($this->request->get['customer_child_id'])) { $this->redirect($this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . $url, 'SSL')); }
		
		if (!isset($this->request->get['customer_child_doctor_id'])) {
			$this->data['action'] = $this->url->link('doctor/childdoctor1/insert', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $this->request->get['customer_child_id'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('doctor/childdoctor1/update', 'token=' . $this->session->data['token'] . '&customer_child_id=' . $this->request->get['customer_child_id'] . '&customer_child_doctor_id=' . $this->request->get['customer_child_doctor_id'] . $url, 'SSL');
		}

    	$this->data['cancel'] = $this->url->link('doctor/childdoctor1', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['customer_child_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$child_info = $this->model_doctor_childdoctor1->getChildDoctor($this->request->get['customer_child_id']);
...
#!#####
		...
		if (isset($this->request->post['doctor_id'])) {
			...
			$this->data['doctor_id'] = $child_info['doctor_id'];
      		...
      		$this->data['doctor_id'] = $this->user->getId();
    	}	
		
		$nar = array(23,24,29,30,31,32);
		for ($i=1;$i<=33;$i++) {
...
*/
		...
		*/
		...
		$this->template = 'doctor/childdoctor1_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'doctor/childdoctor1')) {
