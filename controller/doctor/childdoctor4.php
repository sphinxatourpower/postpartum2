<?php    
class ControllerDoctorChildDoctor4 extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('doctor/childdoctor4');
		 
		$this->document->setTitle($this->language->get('heading_title'));
		
		$this->load->model('doctor/childdoctor4');
	...
	public function word() {		
		...
		if ($customer_info) { 
			...
			$doctor4_infos = $this->model_doctor_childdoctor4->getCustomerDoctor4s($this->request->get['customer_id']);
			...
			foreach ($doctor4_infos	as $kk => $doctor4_info) {
				$this->load->model('service/customer_service');
				$doctor_name = $this->model_service_customer_service->getCustomerServiceName($doctor4_info['doctor_id']);

				$this->model_tool_worddoc->addParagraph("3E婦產科醫師巡診單：".$doctor4_info['date_added']."&#09;&#09;醫師：".$doctor_name, $titleFormat);
				...
				$this->model_tool_worddoc->addParagraph('');
					...
					if ($doctor4_info['chk1']<>'') {
						$this->model_tool_worddoc->addParagraph("產婦意識狀況為：".$doctor4_info['chk1']);
					}
					if ($doctor4_info['chk2']<>'') {
						$this->model_tool_worddoc->addParagraph("產婦乳房狀況為：".$doctor4_info['chk2']);
					}
					if ($doctor4_info['chk3']<>'') {
						$this->model_tool_worddoc->addParagraph("產婦乳暈狀況為：".$doctor4_info['chk3']);
					}
					if ($doctor4_info['chk4']<>'') {
						$this->model_tool_worddoc->addParagraph("宮底位置為：".$doctor4_info['chk4']);
					}
					if ($doctor4_info['chk5']<>'') {
						$this->model_tool_worddoc->addParagraph("產婦生產方式為：".$doctor4_info['chk5']);
					}
					if (is_array($c6 = unserialize($doctor4_info['chk6'])) AND !empty($c6)) {
						$text6 = '';
					...
					if (is_array($c7 = unserialize($doctor4_info['chk7'])) AND !empty($c7)) {
						$text7 = '';
					...
					if (is_array($c8 = unserialize($doctor4_info['chk8'])) AND !empty($c8)) {
						$text8 = '';
					...
					if (is_array($c9 = unserialize($doctor4_info['chk9'])) AND !empty($c9)) {
						$text9 = '';
					...
					if ($doctor4_info['chk10']<>'') {
						$this->model_tool_worddoc->addParagraph("產婦有服藥，藥物為：".$doctor4_info['chk10']);
					}
					if ($doctor4_info['supplement']<>'') {
						$this->model_tool_worddoc->addParagraph("特別補充事項：<br />".nl2br($doctor4_info['supplement']));
					}
					
					if ($kk+1 <> count($doctor4_infos)) {
  	...
  	public function store() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_doctor_childdoctor4->addCustomerDoctor4($this->request->post);
			...
			$this->redirect($this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			//$this->redirect($this->url->link('doctor/childdoctor4/insert', 'token=' . $this->session->data['token'] . '&customer_id='. $this->request->get['customer_id'] .  $url, 'SSL'));
		}
...
*/			
  	...
  	public function delete() {
    	...
    	if (isset($this->request->get['id']) && $this->request->get['id']<>'' && $this->validateDelete()) {
			$this->model_doctor_childdoctor4->deleteChildDoctor4($this->request->get['customer_id'],$this->request->get['id']);
			...
			$this->redirect($this->url->link('doctor/childdoctor4/insert', 'token=' . $this->session->data['token'] . $url, 'SSL'));
  	...
  	private function getList() {	
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . $url, 'SSL'),
    	...
    	foreach ($results as $result) {
			$form4s = $this->model_doctor_childdoctor4->getCustomerDoctor4s($result['customer_id']);
			
			$action = array();
			
			$action[] = array(
				'text' => $this->language->get('text_manage'),
				'href' => $this->url->link('doctor/childdoctor4/insert', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
			);				
			
			if (!empty($form4s)) {
				$action[] = array(
					'text' => '下載列印',
					'href' => $this->url->link('doctor/childdoctor4/word', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] .  $url, 'SSL')
				);	
			}
		...
		$this->data['sort_name'] = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_room_id'] = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . '&sort=room_id' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		$this->data['sort_date_leave'] = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . '&sort=c.date_leave' . $url, 'SSL');
		$this->data['sort_service'] = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . '&sort=service' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'doctor/childdoctor4_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		$this->data['button_add_childdoctor4'] = $this->language->get('button_add_childdoctor4');
		...
		$this->data['tab_childdoctor4'] = $this->language->get('tab_childdoctor4');
		$this->data['tab_childdoctor4form'] = $this->language->get('tab_childdoctor4form');
		...
		if (isset($this->request->get['customer_child_doctor4_id'])) {
			$this->data['customer_child_doctor4_id'] = $this->request->get['customer_child_doctor4_id'];
		} else {
			$this->data['customer_child_doctor4_id'] = 0;
		}
		...
		*/
		...
		if (!isset($this->request->get['customer_id'])) { $this->redirect($this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . $url, 'SSL')); }
		
		$this->data['action'] = $this->url->link('doctor/childdoctor4/store', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');

    	$this->data['cancel'] = $this->url->link('doctor/childdoctor4', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
#!#####
		if (isset($this->request->post['childdoctor4form'])) { 
      		$this->data['childdoctor4forms'] = $this->request->post['childdoctor4form'];
		} elseif (!empty($this->request->get['customer_id'])) {
			$this->data['childdoctor4forms'] = $this->model_doctor_childdoctor4->getCustomerDoctor4s($this->request->get['customer_id']);
		} else {
			$this->data['childdoctor4forms'] = array();
    	}	
...
*/
		...
		*/
		...
		$this->template = 'doctor/childdoctor4_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() {
    	if (!$this->user->hasPermission('modify', 'doctor/childdoctor4')) {
  	...
  	private function validateDelete() {
    	if (!$this->user->hasPermission('delete', 'doctor/childdoctor4')) {
      		$this->error['warning'] = $this->language->get('error_permission');
    	}	
