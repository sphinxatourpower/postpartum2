<?php
class ControllerReportStat1 extends Controller { 
	public function index() {  
		//$this->config->set('config_admin_limit', '20');
		$this->load->language('report/stat1');
		...
		*/
		...
		if (isset($this->request->get['page'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('report/stat1', 'token=' . $this->session->data['token'] . $url, 'SSL'),
		...
		$order_total = $this->model_report_stat1->getTotalNurse2cs($data);
		
		$results = $this->model_report_stat1->getNurse2cs($data);
		...
		if (!empty($results)) {
			foreach ($results as $rk => $result) {		
				...
				$this->data['nurse2cs'][] = array(
					...
					'25'   => $result['chk25'],
					'26'   => $result['chk26'],
					'27'   => "(收縮壓)".$result['chk27a']."<br />(舒張壓)".$result['chk27b']
	...
	//				'products'   => $result['products'],
		...
		*/
		...
		$pagination->url = $this->url->link('report/stat1', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		
		$this->data['viewchart'] = $this->url->link('report/chart1', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
//		$this->data['filter_order_status_id'] = $filter_order_status_id;
		...
		$this->template = 'report/stat1.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
